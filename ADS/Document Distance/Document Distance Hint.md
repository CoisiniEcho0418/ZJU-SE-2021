# Document Distance 提示

- 本题提到需要处理单词的变形形式，需要用到词干提取算法。为了避免使用不同的词干提取算法导致出现不同解的情况，此处提供Porter词干提取算法的代码实现。将结构、函数声明部分放在定义变量前，将结构、函数定义部分放在最后即可。
...
- 本题中包含若干文本，其中文本有标点符号连接两个单词没有空格的情况，此处给出读入*仅包含字母、数字、井号的字符串*的代码实现。

```c
void readstr(char* s) {
	char ch = 0;
    int p = 0;
	while((!isalnum(ch) && ch!='#') && ch!=EOF) ch = getchar();
	while(( isalnum(ch) || ch=='#') && ch!=EOF) {
...
string readstr() {
	char ch = 0;
	string s = "";
	while((!isalnum(ch) && ch!='#') && ch!=EOF) cin.get(ch);
	while(( isalnum(ch) || ch=='#') && ch!=EOF) {
        /*if(isupper(ch)) ch=tolower(ch);*/ 
        s = s+ch;
        cin.get(ch);

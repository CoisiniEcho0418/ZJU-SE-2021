...
class StuHome extends React.Component{
    constructor(props) {
        super(props);
        let type = localStorage.getItem("type") || 'stu';
        let id = localStorage.getItem("username") || 'Klee';
        this.state = {
            get_info_url: "http://localhost:5000/userinfo",
            course_url: "http://localhost:5000/studyCourse",
            user_type: type,
            user_id: id,
            chosen_item: "notice",
            courses: [],
    ...
    componentWillMount(){
        fetch(this.state.course_url + '?userName=' + this.state.user_id, {
                method: "GET",
                mode: "cors",
            }
        )
        ...
        .then(data => {
            ...
            {'课程标识符': 4, '课程名称': '服务科学导论'}, 
            {'课程标识符': 5, '课程名称': '软件工程基础'}]})
            console.log(this.state.course_url + '?userName=' + this.state.id)
        }).catch(function (e) {
            console.log(e);
        });
    }
    ...
    show_center_title(){
        if(this.state.chosen_item == "notes")
            return <div class={'center-title-bar'}>学习笔记 Notes</div>
        else
    ...
    render(){
        return (
            <div class="uhp">
                ...
                <div class="uhp-right">
                    <div class={"courses-title-bar"}>在学课程 COURSES</div>
                    {this.get_courses()}
                </div>
            </div>
        )

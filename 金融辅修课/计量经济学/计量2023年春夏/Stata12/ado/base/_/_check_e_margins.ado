...
program _check_e_margins, rclass
	...
	if "`OPTSNOT'" == "_ALL" {
		di as err "`cmdname' not appropriate after `e(cmd)'"
		exit 322
	}
	...
	if `:length local OPTS' == 0 {
		if `DEFNOT' {
			if strtrim(`"`0'"') == "," {
				di as err "default predict option " _c
				di as err "not appropriate with `cmdname'"
				exit 322
			}
		}
	...
	if `:length local OPTS' {
		di as err ///
		"e(marginsok) and e(marginsnotok) are in conflict"
		error 322
	}
	capture syntax anything(id="command name" name=cmdname) [, `OPTSNOT' *]
	if c(rc) {
		di as err "problem parsing options from e(marginsnotok)"
		exit c(rc)
	}
	local NOT 0
	...
	if `NOT' {
		if `:length local NOTOPT' {
			di as err "predict option `NOTOPT' " _c
		}
		else {
			di as err "default predict option " _c
		}
	...
	if c(rc) {
		di as err "problem parsing options from e(marginsok)"
		exit c(rc)
	}
	local ok 0

...
program _exactreg_replay
	...
	if e(k_groups) > 1 {
		...
		di in gr `"Group variable: `=abbrev("`e(groupvar)'",24)'"' /// 
		 _col(`=`nsk'-4') "Number of groups   =  " in ye %8.0g      ///
		 e(k_groups) _n
		if "`e(binomial)'" != "" {
		...
		di in gr _col(`=`nsk'-4') "Obs per group: min =  " ///
		 in ye %8.0g `nmin' _n _col(`=`nsk'+11') in gr "avg =  "   ///
		 in ye %8.1f `navg' _n _col(`=`nsk'+11') in gr "max =  "   ///
		 in ye %8.0g `nmax' 
	}
	else {
		local ns1 = 14

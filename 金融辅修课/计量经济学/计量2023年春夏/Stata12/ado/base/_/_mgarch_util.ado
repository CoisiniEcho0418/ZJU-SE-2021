...
program define _mgarch_coef_table_header
	...
	local crtype = ///
	    upper(substr(`"`e(crittype)'"',1,1)) + substr(`"`e(crittype)'"',2,.)
	...
	di `osmpl' ///
	    _col(52) "{txt}Number of obs" _col(68) "={res}" _col(70) %9.0g e(N)

	if !missing(e(df_r)) {
	    local model _col(52) "{txt}F({res}" %3.0f e(df_m) "{txt},{res}" ///
	    	%6.0f e(df_r) "{txt})" _col(68) "={res}" _col(70) %9.2f e(F)
	    local pvalue _col(52) "{txt}Prob > F" _col(68) "={res}" _col(73) ///
		...
		local model _col(52) `"{txt}`chitype' chi2({res}"' e(df_m) ///
		    "{txt})" _col(68) "={res}" _col(70) %9.2f e(chi2)
		local pvalue _col(52) "{txt}Prob > chi2" _col(68) "={res}" ///

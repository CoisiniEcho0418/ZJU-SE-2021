...
program _prefix_explist, sclass
	version 9
	syntax [anything(name=exp_list id="exp_list" equalok)],	///
		stub(name) [ GEQname(name) default EDEFaultonly ]

	if `"`exp_list'"' != "" {
		if  `"`default'"' != "" {
			di as err ///
"option default is not allowed with an expression list"
			exit 198
		}
	}
	else if `"`default'`edefaultonly'"' != "" {
		local eb b
		local props `"`e(properties)'"'
		if `:list eb in props' {
			local exp_list _b
		}
		else if "`edefaultonly'" == "" {
			local slist : r(scalars)
			foreach s of local slist {
				local exp_list `"`exp_list' `s'=(r(`s'))"'
			}
		}
		if `"`exp_list'"' == "" {
			di as err "expression list required"
			exit 100
		}
	}

...
"estimation command error: `e'(k_eexp) or `e'(k_exp) is invalid"
	...
	if `"`command'"' != "" {
		local c1 = 1+`col1'-length("command")
	...
	forvalues k = 1/`K' {
		...
		local leq : length local eq
		local lna : length local name
		...
		if `"`exp'"' != "" {
			`blank'
			local blank
			local c1 = 1+`col1'-length("`eq'`name'")-`delta'
			di "{p2colset `c1' `col2' `col2i' 2}{...}"
			if "`eq'" != "" {
...
program ListElementSizes
	...
	forval i = 1/`size' {
		gettoken str 0 : 0
		matrix `b'[1,`i'] = min(`maxlen',`: length local str')
	}
end

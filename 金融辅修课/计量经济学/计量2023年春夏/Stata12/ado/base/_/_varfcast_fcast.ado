*! version 1.5.2  12feb2010
program define _varfcast_fcast
	version 8.0
...
/* To deal with timeseries operators in depvar names
   there are three parallel lists
	...
	endog_ts contains names of temporary variables generated
		...
		labeled.
   note that matrix `b_ts' is created from e(b) by replacing all 
 ...
 */
	...
	if "`r(panelvar)'" != "" {
		di as error "cannot forecast with panel data"
		exit 198 
	}	
 ...
 */
	...
	if "`fbegin'" != "" {
		tempvar gbob gbob2 
		tempname max1 max2
		gen `gbob'=cond(`t'==`fbegin'-`: char _dta[_TSdelta]',_n,0)
		gen `gbob2'=sum(`gbob')
		qui sum `gbob'
		local max1=r(max)
		qui sum `gbob2'
...
/* rename forecasted variables  */
	if "`fbegin'" == "" {
		...
		local fbegins : di `tsfmt' `t'[`fbegin']
	}
	else {
		local fbegins : di `tsfmt' `fbegin'
	}
	local fbegins = ltrim("`fbegins'")
	local i 1
	foreach v of local eqlist {
		if "`replace'" == "replace" {
			foreach typ in none _LB _UB _SE {
				if "`typ'" == "none" local typ ""
				capture drop `prefix'`v'`typ'
			}	
		}
			
		local vorig : word `i' of `endog'
		local dyn  "dyn(`fbegins')"
		local lbl "`prefix'`v', `dyn'"
		qui rename `tv`i'' `prefix'`v'
		char define `prefix'`v'[observed] "`vorig'"
		label variable `prefix'`v' "`lbl'"
		local i = `i' + 1
	}
...
if `bssize' >= `free_m' {
	...
	di as err "{p 4 4}There are `free_m' bytes free, "/*
		*/ "but the bootstrapped dataset will "	/*
		*/ "require `bssize' bytes.{p_end}"
	di as err "{p 4 4}At present you have about "	///
	...
	exit 901
		...
		noi _varbsf using "`bsfile'", reps(`reps') step(`steps') /*
			...
			*/ fsamp(`fsamp') `dots' prefix(`prefix') `bsquiet'
		qui drop _all
		qui use "`bsfile'"
		if "`bscentile'" != "" {
		...
		forvalues i = 1(1)`steps' {
			foreach v of local eqlist {
				if `i' == 1 {
					qui gen double `prefix'`v'_LB = .  
					qui gen double `prefix'`v'_UB = .
					qui gen double `prefix'`v'_SE = .
				}	

				if "`bscentile'" != "" {
					qui replace `prefix'`v'_LB = 	/*
						*/ ``v'L`i'' 		/*
						*/ if _n==`lastob'+`i'
					qui replace `prefix'`v'_UB =	/*
						*/	``v'U`i'' 	/*
						*/ if _n==`lastob'+`i'
					qui replace `prefix'`v'_SE =	/*
						*/ ``v'STD`i'' 		/*
						*/ if _n==`lastob'+`i'
				}
				else {
...
label variable `prefix'`v'_LB `"`=strsubdp("`level'")'% LB for `prefix'`v'"'
label variable `prefix'`v'_UB `"`=strsubdp("`level'")'% UB for `prefix'`v'"'
label variable `prefix'`v'_SE "SE for `prefix'`v'"
...
label variable `prefix'`v'_SE "SE for `prefix'`v'"
	...
	foreach v of local eqlist {
		local created `created' `prefix'`v' 
	}

	if "`bs'`bsp'" != "" {
		foreach v of local eqlist {
local created `created' `prefix'`v'_LB `prefix'`v'_UB `prefix'`v'_SE 
		}
	}
	else {
		if "`se'" == "" {
...
program define FCAST2
	...
	*/	
	...
	if "`ta'" == "" {
		di as error "you must tsset your data before using fcast"
		exit 198
	}
...
program define getSEb
	...
	*/	
	...
	/* make B which is (Kp+1) x (Kp+1) (see Lutkepohl(1993) section 3.5.2 */
	...
	forvalues i = 0(1)`mlag' {
		if `i' == 0 {
			...
			if "`cons'" != "" {
				matrix `Bt' = J(1,`eqs',0) \ `A'`i' \   /*
					*/ nullmat(`Btt') 
			}
			else {
				matrix `Bt' = `A'`i' \ nullmat(`Btt') 
			}
...
/* See Note 1 below */	
		...
		foreach var of local eqlist {
			...
			qui replace `prefix'`var'_LB=			///
				`prefix'`var' -`norm'*`stderr' in `predob'
			qui replace `prefix'`var'_UB=			///
				`prefix'`var' +`norm'*`stderr' in `predob'
			qui replace `prefix'`var'_SE=`stderr' in `predob'
			local i = `i' + 1

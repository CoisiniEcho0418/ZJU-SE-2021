...
// Estimation engine for xtmelogit/xtmepoisson
	...
	local globopts `"`globopts' BINomial(string) MLe VARiance "'
	...
	local globopts `"`globopts' noLRtest noLOg"'
...
// parse
	...
	_mixed_parseifin `=`cmd_n'+1' `cmds' `"`glob_if' `glob_in'"'
...
// obtain re estimates if specified and exit
	...
	if `"`getblups'"' != "" | `"`getblupses'"' != "" {
		...
		CheckBlups `getblups'`getblupses'
		if `"`getblupses'"' != "" {
			local se se
		}
		SetBlupMats `se'
		GetBlups "`se'" `getblups'`getblupses'
		exit
	}
...
// Begin -ml- iterations 
	...
	ml model d0 glmixed_ll (`depname' = `varll_0', `constant_0')	///
		...
		init(`init', copy) search(off) `dscale' `log'
...
program SetModel
	...
	local l = length(`"`f'"')
	if `"`f'"'==substr("logistic",1,max(`l',1)) {
...
di as err `"binomial denominator out of range"'
	...
	else if `"`f'"'==substr("poisson",1,max(`l',1)) {
...
program SetBlupMats 
	...
	if `"`e(cmd)'"' != "xtmelogit" & `"`e(cmd)'"' != "xtmepoisson" {

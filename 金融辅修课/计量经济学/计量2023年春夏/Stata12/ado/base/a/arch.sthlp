...
{viewerjumpto "Saved results" "arch##saved_results"}{...}
{viewerjumpto "References" "arch##references"}{...}
{title:Title}

{synoptset 11}{...}
{synopt:{manlink TS arch} {hline 2}}Autoregressive conditional
...
{synopt :{it:{help arch##display_options:display_options}}}control column
...
{p_end}
{p 4 6 2}
{opt by}, {opt rolling}, {opt statsby}, and {cmd:xi} are allowed; see
{help prefix}.{p_end}
{p 4 6 2}
...
{opth nparchk(numlist)} specifies lags of the two-parameter
   term a|e_t-k|^p; this is a variation on {opt nparch()} with k
...
{opth het(varlist)} specifies that {it:varlist} be included in the
   ...
   {help arch##J1985:Judge et al. (1985)}.
...
see below for information on {cmd:gtolerance()}.
...
criterion is met.
The default gradient tolerance for {opt arch} is {cmd:gtolerance(0.5)}.
...
of values, or coefficient name value pairs; see {manhelp maximize R}.  In
...
{pstd}
The following option is available with {opt arch} but is not shown in the
dialog box:
...
{pstd}
{cmd:arch} saves the following in {cmd:e()}:

...
different type, you must drop the identifier first and then recreate it; see
...
reinitialized because they need not be recreated, being shared.  (That is
...
{pstd}
To be precise, {it:newclassname} inherits all of the member variables from the
...
{pstd}
All of the examples above are valid because either (1) a new identifier is
...
{pstd}
All of the above will make {cmd:.}{it:d}{cmd:.}{it:e} unique because what is
...
{bf:inheritance}:  the ability to define a class in terms of one (single
    ...
    class inherits all of the member variables and member programs of the base
    class.

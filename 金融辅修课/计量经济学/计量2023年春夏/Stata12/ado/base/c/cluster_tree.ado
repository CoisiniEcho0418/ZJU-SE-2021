...
program Dendrogram
	...
	if "`quick'" != "" {
		GenQuickData `newid' `heightv' using `"`dendrodata'"'
	}
	else {
		// NOTE: GenCenterData modfies the `newid' variable, so make
		// sure to perform all calculations that require this variable
		// prior to the following call.
		GenCenterData `newid' `heightv' using `"`dendrodata'"'
	}
...
program GenQuickData
	...
	tempname post
	postfile `post' x1 y1 x2 y2 `using'

	local imax 1
	local nm1 = `n' - 1
	forval i = 1/`nm1' {
		post `post' (`x'[`i']) (0) (`x'[`i']) (`y'[`i'])
		local ip1 = `i' + 1
		forval j = `ip1'/`n' {
			if `y'[`j'] >= `y'[`i'] {
				local tox `j'
		...
		post `post' (`x'[`i']) (`y'[`i']) (`x'[`tox']) (`y'[`i'])
		if `y'[`imax'] < `y'[`i'] {
			local imax `i'
		}
	}
	post `post' (`x'[`n']) (0) (`x'[`n']) (`y'[`imax'])

...
// -estat- common commands for -discrim-
	...
	else if `"`key'"' == substr("grsummarize",1,max(3,`lkey')) {
		GroupSumm `rest'
	}
	else if `"`key'"' == substr("list",1,max(2,`lkey')) {
		List `rest'
	}
	else if `"`key'"' == substr("classtable",1,max(5, `lkey')) {
		ClassTable `rest'
	}
	else if `"`key'"' == substr("errorrate",1,max(3, `lkey')) {
		ErrRate `rest'
	}
	else {
		di as err `"invalid subcommand `key'"'
		exit 321
	}
...
program ErrRate, rclass
	...
	if `"`pp'"' != "" & "`looclass'`class'`prvars'" == "" {
		di as err "pp requires prvars() if cvar() is specified"
		exit 198
	}
	...
	if "`looclass'" != "" {
		...
		if r(sum) < 1 {
			di as err ///
		    "observations restricted to e(sample) with looclass option"
			error 2000
		}
	}
	...
	if "`class'`looclass'" != "" { // get group and prob. predictions
		...
		if `: list sizeof prvars' != `ngps' {
			di as err "prvars() invalid; `ngps' variables " ///
...
program GroupSumm, rclass
	...
	di as text "Summarized by `grpvar'"
...
/* and hold weight since that will get wiped by syntax */
	...
	if "`novarlist'" != "" & "`varlist'`varlist2'" != "" {
		di as err "novarlist may not be specified with varlist"
		exit 198
	}
	if `"`varlist'`varlist2'"' != "" {
		local hvar `varlist'
		...
		if _rc {
			di as err "varlist(): invalid syntax"
			exit _rc
		}
		local 0 , `anything'
		...
		if "`format'" != "" & "`format1'" != "" {
			di as err "varlist(): format() may be specified " ///
				"only once"
			exit 198
		}
		if "`format1'" != "" {
		...
		if `"`format'"' != "" {
			if "`none'" != "" {
				di as error "varlist(): format() not valid " ///
					"with none"
				exit 198
			}
...
/* Next work on classification */
	if ("`classification'"!= "" | "`classification2'" != "")	///
			 & "`noclassification'" != "" {
		di as err "classification and noclassification may not " ///
			"be combined"
		exit 198
	}
	if "`noclassification'"== "" {
		...
		capture syntax [anything] [, FORmat1(str)]
		if _rc {
			di as err "classification(): invalid syntax"
			exit _rc
		}
		local 0 , `anything'
		...
		if "`copt'" != "" & "`none'" != "" {
			di as err "classification(): none may not be "	///
				"specified with looclass, group() or cvar()"
			exit 198
		}
		if "`looclass'" != "" & "`lcvar'" != "" {
			di as err "classification(): looclass may not be " ///
				"specified with lcvar()"
			exit 198
		}
		if "`lcvar'" != "" {
		...
		if "`group'" != "" & `"`nogroup'"' != "" {
			di as err "classification(): group() may not be " ///
				"specified with nogroup"
			exit 198
		}
		...
		if "`format'" != "" & "`format1'" != "" {
			di as err "classification(): format() may be " ///
				"specified only once"
			exit 198
		}
		if "`format1'" != "" {
		...
		if "`format'" != "" {
			if "`none'" != "" {
				di as err "classification(): format() "	///
					"may not be specified with none"
				exit 198
			}
...
/* Next probabilities */
	if ("`probabilities'"!= "" | "`probabilities2'" != "")	///
			 & "`noprobabilities'" != "" {
		di as err "probabilities and noprobabilities may not " ///
			"be combined"
		exit 198
	}
	if "`noprobabilities'"== "" {
		...
		capture syntax [anything] [, FORMat1(str)]
		if _rc {
			di as err "probabilities(): invalid syntax"
			exit _rc
		}
		local 0 , `anything'
		...
		if "`popt'" != "" & "`none'" != "" {
			di as err "probabilities(): none may not be "  ///
				"specified with loopr, prvars() or lprvars()"
			exit 198
		}
		if "`loopr'" !="" & "`lprvars'"!="" {
			di as err "probabilities(): loopr may not be " ///
				"specified with lprvars()"
			exit 198
		}
		if "`lprvars'"!= "" {
			local loopr loopr
		...
		if "`format'" != "" & "`format1'" != "" {
			di as err "probabilities(): format() may be " ///
				"specified only once"
		}
		if "`format1'" != "" {
			local format `format1'
		...
		if "`format'" != "" {
			if "`none'" != "" {
				di as err "probabilities(): format() " ///
					"may not be specified with none"
				exit 198
			}
...
/* Next weight */
	...
	if ("`weight2'" != "" | "`weight'" != "")& "`noweight'" != "" {
		di as err "weight and noweight may not be specified together"
		exit 198
	}
	...
	if "`noweight'" == "" {
		...
		if "`format1'" != "" & "`format'" != "" {
			di as err "weight(): format() may be specified " ///
				"only once"
			exit 198
		}
		if "`format1'" != "" {
	...
	if "`id'" != "" {
		...
		local idvar `options'
		if "`format1'" != "" & "`format'" != "" {
			di as err "id(): format() may be specified only once"
			exit 198
		}
		if "`format1'" != "" {
			local format `format1'
...
// Now handle misclassified option
	if `"`misclassified'"' != "" {
		...
		if "`grvar'" == "" & "`e(groupvar)'" == "" {
			di as err ///
			    "option misclassified requires a group variable"
			exit 459
		}
		if "`cvar'" == "" & "`lcvar'" == "" {
...
program ClassTable, rclass
	...
	if "`prvars'" != "" & "`probabilities'" == "" {
		di as err "prvars() allowed only with option probabilities"
		exit 198
	}
	...
	if `"`grpvar'"' != "" & "`class'" != "" {
		di as error "class not valid with cvar()"
		exit 198
	}

	if `"`grpvar'"' != "" & "`looclass'" != "" {
		di as error "looclass not valid with cvar()"
		exit 198
	}
	...
	if `"`noupdate'"'!= "" & "`e(subcmd)'" != "knn" {
		di as error "noupdate only valid with discrim knn"
		exit 198
	}
	if `"`noupdate'"'!= "" & "`e(mahalanobis)'" == "" {
		di as error "noupdate only valid with Mahalanobis distance"
		exit 198
	}
	if `"`looclass'"' =="" & "`noupdate'"!="" {
		di as error "noupdate only valid with looclass"
		exit 198
	}
	...
	if "`cvar'" != "" {
		...
		if `ng' > e(N_groups) {
			di as err "{p}variable `cvar' not a classification" ///
		...
		//==================================================//
		if `"`priors'"' != "" & `"`nopriors'"' != "" {
			di as error "when cvar() is specified priors "	///
				"may not be specified with nopriors"
			exit 198
		}
		if `"`priors'"' == "" {
...
program define FormatChk
	...
	if _rc {
		di as err "`optname'(): invalid format"
		exit 198
	}
end

...
{viewerjumpto "Saved results" "discrim knn##saved_results"}{...}
{title:Title}

{p2colset 5 25 27 2}{...}
{p2col:{manlink MV discrim knn} {hline 2}}kth-nearest-neighbor discriminant
	analysis
{p_end}
{p2colreset}{...}
...
{p 8 20 2}
{cmd:discrim} {cmd:knn} {varlist} {ifin} {weight}{cmd:,}
...
{synopt:{cmd:s2d(}{cmdab:st:andard)}}convert similarity to dissimilarity:
		d(ij)=sqrt(s(ii)+s(jj)-2s(ij)), the default{p_end}
...
wide selection of similarity and dissimilarity measures is available; see the
{helpb discrim knn##measure():measure()} option.
...
{help measure_option:[MV] {it:measure_option}} are supported
...
{p2colset 13 25 27 2}{...}
{p2col:{cmd:standard}}d(ij) = sqrt(s(ii)+s(jj)-2s(ij)) = sqrt(2{1-s(ij)}){p_end}{p2col:{cmd:oneminus}}d(ij) = 1-s(ij){p_end}
{p2colreset}{...}
...
computing dissimilarities.  The data is transformed via the Cholesky
...
{pstd}
{cmd:discrim knn} saves the following in {cmd:e()}:

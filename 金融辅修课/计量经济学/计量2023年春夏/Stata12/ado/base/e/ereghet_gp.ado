...
program define ereghet_gp, sort
	version 7, missing
...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		gen `vtyp' `varn'=((0.5)^(-`theta')-1)*exp(-`xb') /*
			*/ /`theta' if `touse'
...
*/ "(option log median time assumed; predicted median log time)"
		...
		gen `vtyp' `varn'= ln(((0.5)^(-`theta')-1)*exp(-`xb') /*
			*/ /`theta') if `touse'
	...
	if "`type'"=="hazard" {
		...
		qui gen double `ff'= `kk'/(1+`theta'*`kk'*`t') if `touse'
	...
	if "`type'" =="surv" {
		...
		qui gen double `ff'= 1+`theta'*exp(`xb')*`t' if `touse'
		qui gen double `ff0'= 1+`theta'*exp(`xb')*`t0' /*
			*/ if `t0'>0 & `touse'
		qui replace `ff' = `ff'^(-1/`theta')
		qui replace `ff0' = `ff0'^(-1/`theta') /* 
			*/ if `t0'>0 & `touse'

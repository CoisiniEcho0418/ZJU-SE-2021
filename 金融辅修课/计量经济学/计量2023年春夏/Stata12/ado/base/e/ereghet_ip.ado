...
program define ereghet_ip, sort
	version 7, missing
...
"option scores is not allowed with shared frailty"
		...
		if "`e(frm2)'" == "time" {
			...
			// temporarily remap the coefficients
			AFT
		}
		ml score `0'
		if "`e(frm2)'" == "time" {
			foreach var in `r(scorevars)' {
				quietly replace `var' = -`var'
			}
		}
		macro drop EREG*
		exit
...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		if "`e(frm2)'"=="time" {
			qui replace `xb' = -`xb' if `touse'
		}
		gen `vtyp' `varn'=((1-`theta'*ln(0.5))^2 - 1)/(2*`theta') /*
			*/ *exp(-`xb') if `touse'
...
*/ "(option log median time assumed; predicted median log time)"
		...
		gen `vtyp' `varn'= ln(((1-`theta'*ln(0.5))^2 - 1)/ /*
			*/ (2*`theta')*exp(-`xb')) if `touse'
	...
	if "`type'"=="hr" {
		di as err "unconditional hazard ratios not available"
		exit 198
	}
	...
	if "`type'"=="hazard" {
		...
		qui gen double `kk' = exp(`xb') if `touse'
		qui gen double `ff'= `kk'/ /*
			*/ sqrt(1+2*`kk'*`t'*`theta') if `touse'
	...
	if "`type'" =="surv" {
		...
		qui gen double `ff'= 1+2*`theta'*exp(`xb')*`t' if `touse'
		qui gen double `ff0'= 1+2*`theta'*exp(`xb')*`t0' /*
			*/ if `t0'>0 & `touse'
		qui replace `ff' = exp((1-sqrt(`ff'))/`theta') if `touse'
		qui replace `ff0' = exp((1-sqrt(`ff0'))/`theta') /* 
			*/ if `t0'>0 & `touse'
	...
	if "`type'"=="mgale" { 
		...
		qui gen double `ff'= /*
		*/ (`d'!=0)+(1/`theta')* /*
		*/ (1-sqrt(1+2*`theta'*exp(`xb')*`t')) if `touse'
		qui replace /*
		*/ `ff'=`ff'-(1/`theta')* /*

...
{viewerjumpto "Saved results" "estimates_save##saved_results"}{...}
{title:Title}

{p2colset 5 27 29 2}{...}
{p2col :{manlink R estimates save} {hline 2}}Save and use estimation results{p_end}
{p2colreset}{...}
...
{title:Menu}
    ...
    {title:estimates save}

{phang2}
{bf:Statistics > Postestimation > Manage estimation results > Save to disk}

    {title:estimates use}

{phang2}
{bf:Statistics > Postestimation > Manage estimation results > Load from disk}
...
if you do have data in memory that look like the original data, it might not
...
{cmd:e(sample)} to 0 ensures that, should you ask for one of them, 
...
you are going to take responsibility for setting {cmd:e(sample)} correctly.
Here we just happen to know that all the foreign observations 
were used, so we can type 
...
{pstd}
{cmd:estimates} {cmd:esample} without the colon saves macro {cmd:r(who)}, 

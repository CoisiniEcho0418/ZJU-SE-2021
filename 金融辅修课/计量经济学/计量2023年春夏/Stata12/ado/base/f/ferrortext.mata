...
string scalar ferrortext(real scalar userec)
{
	...
	if (ec==682) return("could not connect to odbc dsn")
	...
	if (ec==3602) return("invalid file name")
	if (ec==3611 | ec==681) return("too many open files")
	if (ec==3622) return("attempt to write read-only file")

*! version 7.1.9  19apr2007
program define fh_st, rclass 
	version 6, missing

	local vv : display "version " string(_caller()) ", missing:"

	syntax varlist(min=1 max=2) [if] [in] [fw iw] [, p(numlist) /*
		*/ q(numlist) /*
	...
	if `"`strata'"' != `""' {
		if `"`detail'"' != `""' {
			...
			di 
			di in gr `"-> Total"'
			fh_st `varlist' [`weight'`exp'] if `touse' /*
			*/ , by(`by') `t0opt' `idopt' strata(`strata') /*
			*/ `trend' notitle p(`p') q(`q')
			if `"`check'"' != `""' {
				mat `V' = syminv(`V')
				mat `V' = `v'*`V'
				mat `V' = `V'*`v' '

...
{viewerjumpto "ASCII text output specifications" "file##text_output"}{...}
{viewerjumpto "Remarks" "file##remarks"}{...}
{viewerjumpto "Saved results" "file##saved_results"}{...}
{title:Title}

{p2colset 5 17 19 2}{...}
{p2col :{manlink P file} {hline 2}}Read and write ASCII text and binary files{p_end}
{p2colreset}{...}
...
{manhelp insheet D}, {manhelp infile D}, and {manhelp infix D}, which are the
usual ways that data are brought into Stata.  {cmd:file} allows programmers to
read and write both ASCII text and binary files, so {cmd:file} could be used
...
{pstd}
For information on reading and writing sersets, see {manhelp serset P}.
...
{cmd:text} and {cmd:binary} determine how the file is to be treated once it is
    opened.  {cmd:text}, the default, means ASCII text files.
...
Remarks are presented under the following headings:
	...
	{help file##2.1:Writing ASCII text files}
	{help file##2.2:Reading ASCII text files}
{phang2}{help file##2.3:Use of seek when reading and writing ASCII text files}{p_end}
...
{marker 2.1}{...}
{title:Writing ASCII text files}
...
{marker 2.2}{...}
{title:Reading ASCII text files}
...
display lines exactly as stored; see {manhelp smcl P}.  We also used the 
{mansection P macroRemarksmacval():{bf:macval()}} macro function to obtain what
was in {cmd:`line'} without recursively expanding the contents of line.


{marker 2.3}{...}
{title:Use of seek when reading and writing ASCII text files}
...
even then, it is seldom used with ASCII text files.
...
PowerPC-based Mac, we would discover that we could not read the file.  Intel
computers write multiple-byte numbers with the least-significant byte first;
PowerPC-based computers write the most-significant byte first.  Who knows what
...
than 165,199 characters.  The solution is to obtain the length of the string
...
{p 5 9 2}
	...
	macro extended function {cmd:length}:

	    {cmd:local length : length local mystr}
	    {cmd:file write} {it:handle} {cmd:%`length's `"`mystr'"'}

{p 9 9 2}
	See {it:{help extended_fcn##parsing_desc:Macro extended functions for parsing}} in 
...
{pstd}
{cmd:file read} saves the following in {cmd:r()}: 
...
(33{cmd:maxvar}+199 for Stata/MP and Stata/SE, 165,199 for Stata/IC, 8,680 for
Small Stata) characters of the line were returned and that the next {cmd:file}
...
{pstd}
{cmd:file seek} saves the following in {cmd:r()}:
...
{pstd}
{cmd:file query} saves the following in {cmd:r()}:

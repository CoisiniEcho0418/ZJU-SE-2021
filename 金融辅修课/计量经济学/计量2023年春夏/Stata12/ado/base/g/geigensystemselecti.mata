...
void geigensystemselecti(numeric matrix A, B, real vector index, 
		numeric matrix X, w, beta)
{
	...
	if(isfleeting(A) && isfleeting(B)) {
		_geigenselecti_la(A, B, ., X, w, beta, index, "R")
	}
	else if(isfleeting(A)) { 
		_geigenselecti_la(A, kb=B, ., X, w, beta, index, "R")
	}
	else if(isfleeting(B)) { 
		_geigenselecti_la(ka=A, B, ., X, w, beta, index, "R")
	}
	else { 
		_geigenselecti_la(ka=A, kb=B, ., X, w, beta, index, "R")
	}
}

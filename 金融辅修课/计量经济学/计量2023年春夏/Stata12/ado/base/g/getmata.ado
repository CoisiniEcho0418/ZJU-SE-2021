...
void parse_getlist_paren(`Tr' callerst, `SS' boundlist, `PROBLEM' pr)
{
	...
	toparse = substr(boundlist,   2, .)
	toparse = substr(toparse,     1, strlen(toparse)-1)
			...
			`SR' evarnamelist, `SS' matname, `boolean' existsok)
{
	...
	if (length(evarnamelist)==1) {
		if (substr(evarnamelist, -1, 1) == "*") isexpandlist = 1
	}
	/* ------------------------------------------------------------ */
					/* store matrix info		*/
...
`SR' expand_evarnamelist(`SR' evarnamelist, `RS' n)
{
	...
	if (substr(evarnamelist, -1, 1) != "*") return(evarnamelist)

	basename = substr(evarnamelist, 1, strlen(evarnamelist)-1)
	toret = J(1, n, "")
	for (i=1; i<=n; i++) {
...
void post_variables_i_j(`PROBLEM' pr, `ELEMENT' el, matrix values, `RS' j)
{
	...
	if (isnumeric) {
		...
		up_strlen = (up_strlen<1 ? 
				1 : 
				(up_strlen>255 ? 255 : up_strlen))

		if (el.varexists[j]) {
			orig_strlen = strtoreal(
				substr(st_vartype(varname), 4, .))

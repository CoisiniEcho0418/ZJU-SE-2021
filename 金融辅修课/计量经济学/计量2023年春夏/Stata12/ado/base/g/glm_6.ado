...
comment by wwg:
	...
	code and, moreoever, "family(gam)" produces the description 
...
* di `"                   scale1 |`scale1'|  m |`m'|  mfixed |`mfixied'|"'
...
program define MapFam, rclass /* {tocode | fromcode} <code> <ignored> ... */
	...
	local l = length(`"`f'"')
	if `"`f'"'=="" 					  { local s1 "gau" }
	else if `"`f'"'==substr("binomial",1,`l') 	  { local s1 "bin" }
	else if `"`f'"'==substr("bernoulli",1,`l')	  { local s1 "bin" }
	else if `"`f'"'==substr("gamma",1,max(`l',3)) 	  { local s1 "gam" }
	else if `"`f'"'==substr("gaussian",1,max(`l',3))  { local s1 "gau" }
	else if `"`f'"'==substr("igaussian",1,max(`l',2)) { local s1 "ivg" }
	else if `"`f'"'==substr("inormal",1,max(`l',2))   { local s1 "ivg" }
	else if `"`f'"'=="ivg" 				  { local s1 `"`ivg'"' }
	else if `"`f'"'==substr("normal",1,`l') 	  { local s1 "gau" }
	else if `"`f'"'==substr("nbinomial",1,max(2,`l')) { local s1 "nb" }
	else if `"`f'"'==substr("poisson",1,`l') 	  { local s1 "poi" }
	else { 
...
/*
	...
	(Capitalization indicates minimum abbreviations.)
		...
		IGaussian		ivg
		INormal			ivg
		IVG			ivg	(for backwards capatibility)
		Normal			gau
		NBinomial		nb
		Poisson			poi
*/
...
program define MapLink, rclass /* {tocode | fromcode ..} <code> <coderest> ... */
	...
	if `"`ulink'"'=="" 				{ local s1 }
	else if `"`ulink'"'==substr("identity",1,`l') 	{ local s2 1 }
	...
	else if `"`ulink'"'==substr("logit",1,`l')	{ local s1 "l" }
	else if `"`ulink'"'==substr("probit",1,`l')	{ local s1 "p" }
	else if `"`ulink'"'==substr("cloglog",1,`l')	{ local s1 "c" }
	else if `"`ulink'"'==substr("nbinomial",1,`l')	{ local s1 "nb" }
	else {
	...
	if `"`link'"'=="pow" {
		...
		if `"`fam'"' == "bin" {
			if `"`link'"' == "l" {
				...
				gen double `w' = -`m'*(1-`mu'/`m')*log(1-`mu'/`m')
			}
			else if `"`link'"' == "opo" {

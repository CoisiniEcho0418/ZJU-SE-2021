*! version 3.5.6  27apr2009
program define gnbreg, eclass sort prop(svyb svyj svyr irr ml_score mi)
...
/* Parse. */
	...
	*/ SCore(string) noLOg LNAlpha(varlist numeric fv)		/*
	*/ CRITTYPE(passthru) * ]
...
/* Fit constant-only model. */
	...
	/* Get starting values for constant-only model. */
		...
		*/ init(`b0') search(off) `mlopts' `log' `robust' nocnsnotes

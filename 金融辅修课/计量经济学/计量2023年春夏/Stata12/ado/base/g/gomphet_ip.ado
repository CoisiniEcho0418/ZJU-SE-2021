...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		gen `vtyp' `varn'=ln(`gamma'/(2*`theta')* /*
			*/ ((1-`theta'*ln(0.5))^2 - 1)*exp(-`xb') + 1) / /*
			*/ `gamma' if `touse'
...
*/ "(option log median time assumed; predicted median log time)"
		...
		tempvar xb
		qui _predict double `xb' if `touse', xb `offset'
		gen `vtyp' `varn'=ln(ln(`gamma'/(2*`theta')* /*
			*/ ((1-`theta'*ln(0.5))^2 - 1)*exp(-`xb') + 1) / /*
			*/ `gamma') if `touse'
	...
	if "`type'"=="hazard" {
		...
		qui gen double `ff'= `kk'/sqrt(1+2*`theta'/`gamma'* /*
			*/ (`kk'-exp(`xb'))) if `touse'
	...
	if "`type'" =="surv" {
		tempvar xb ff ff0
		qui _predict double `xb' if `touse', xb `offset'
		qui gen double `ff'=1+2*`theta'/`gamma'*exp(`xb')* /*
			*/ (exp(`gamma'*`t')-1) if `touse'
		qui gen double `ff0'=1+2*`theta'/`gamma'*exp(`xb')* /*

...
of {it:newfilename}{cmd:.}{it:suffix}:
    ...
    {hline 69}
{phang}
{cmd:ps}, {cmd:eps}, and {cmd:pdf} are available with all versions of Stata;
{cmd: png} and {cmd:tif} are available for all versions of Stata except
Stata(console) for Unix; and {cmd:wmf} and {cmd:emf} are available only with
Stata for Windows.

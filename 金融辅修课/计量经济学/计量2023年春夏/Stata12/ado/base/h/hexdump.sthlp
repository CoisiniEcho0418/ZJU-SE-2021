...
{viewerjumpto "Saved results" "hexdump##saved_results"}{...}
{title:Title}

{p2colset 5 20 22 2}{...}
{p2col :{manlink D hexdump} {hline 2}}Display hexadecimal report on file{p_end}
{p2colreset}{...}
...
itself{p_end}
{synopt :{opt tab:ulate}}display a full tabulation of the ASCII characters in
the {opt analyze} report{p_end}
{synopt :{opt noex:tended}}do not display printable extended ASCII characters{p_end}
{synopt :{opt res:ults}}save results containing the frequency with which each
...
tabulation of the ASCII characters also be presented.
...
other saved results, {opt hexdump} save {cmd:r(c0)}, {cmd:r(c1)}, ...,
...
{pstd}
{cmd:hexdump, analyze} and {cmd:hexdump, results} save the following in
{cmd:r()}:

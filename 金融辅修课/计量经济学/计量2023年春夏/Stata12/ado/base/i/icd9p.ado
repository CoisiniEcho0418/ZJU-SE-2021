*! version 1.0.9  28sep2006
program define icd9p
	version 9
	...
	else 	di in red "invalid icd9p subcommand"
end
...
program define Check, rclass
	...
	if substr("`typ'",1,3)!="str" {
		di in red "`varlist' does not contain ICD-9 procedure codes;" /*
	...
	if `bad'==0 {
		...
		*/ "(`varlist' contains valid ICD-9 procedure codes; " /*
		...
		qui count if `prob'==5
		di in gr "    5.  Invalid 1st char (not 0-9)" /*
			*/ _col(49) in ye %11.0gc r(N)
		ret scalar e5 = r(N)
...
program define Lookup
	...
	qui icd9p clean __code9, dots
	di _n in gr _N " match`es' found:"

	local i 1 
	while `i' <= _N { 
		di in ye _col(5) __code9[`i'] _col(14) in gr __desc9[`i']
		local i = `i' + 1
	}
end
...
program define Tabulate
	...
	if `"`subpop'"' != "" { 
		di in red "option subpop() not allowed with icd9p tabulate"
		exit 198
	}
	if `"`generate'"' != "" {
		di in red "option generate() not allowed with icd9p tabulate"
		exit 198
	}
	...
	quietly {
		...
		keep `2' `wgtv' `c'
		icd9p genp `desc' = `c', desc long end
		label var `desc' `"`lbl'"'
	}
...
program define Table
	...
	quietly {
		...
		icd9p gen `desc' = `c', desc long end
		label var `desc' `"`lbl'"'
	}
...
program define IsEl, sclass
	...
	if `"`c'"' == "" { 
		di in red "<nothing> invalid ICD-9 procedure code"
		exit 198
	}
	if index(`"`c'"', ".") { 
		local l = index(`"`c'"', ".")
	...
	if (`"`l'"'<"0" | `"`l'"'>"9") & `"`l'"'!="V" { 
		Invalid `"`c'"' "1st character must be 0-9"
	}
	...
	if (`"`l'"'<"0" | `"`l'"'>"9") & `"`l'"'!="" { 
		Invalid `"`c'"' "4th character must be 0-9"
	}
...
program define FindFile
	capture noi quietly icd9_ff icd9_cop.dta, ado
	if _rc==0 {
		exit
	}
	local rc = _rc 
	di
	di in gr "icd9p needs a dataset that records the valid ICD-9 codes."
	di in gr "That dataset is stored with the icd9p program."
	di in gr `"Type "help icd9p" and see the installation instructions."'
	exit `rc'
end

...
{viewerjumpto "Saved results" "irf_ctable##saved_results"}{...}
{title:Title}

{p2colset 5 24 26 2}{...}
{p2col:{manlink TS irf ctable} {hline 2}}Combine tables of IRFs, 
...
{pstd}
{opt irf ctable} makes a table or a combined table of IRF results.
Each block within a pair of matching parentheses -- each 
({it:spec_k}) -- specifies the information for a specific table.  
{opt irf ctable} combines these tables into one table, unless the 
{opt individual} option is specified, in which case separate tables for each
block are created.

{pstd}
{opt irf ctable} operates on the active IRF file; see
...
{opt noci} suppresses reporting of the confidence intervals for each
    statistic.  {opt noci} is the default when the model was fit by
...
{pstd}
{cmd:irf ctable} saves the following in {cmd:r()}:

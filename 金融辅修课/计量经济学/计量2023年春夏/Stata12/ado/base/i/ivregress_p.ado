...
program define ivregress_p
	...
	if "`scores'" != "" {
		CalcScores `anything' `if' `in' , `options'
		exit
	}
	
	// Take care of xb quickly if we can
	local myopts Residuals STDP STDF Pr(string) E(string) YStar(string)
	_pred_se "`myopts'" `0'
	if `s(done)' {
		exit
	}
	...
	if "`mostopts'" != "" {
		...
		if "`e(estimator)'" == "gmm" {
			di as err "`mostopts' not available with GMM estimator"
			exit 198
		}
		if "`e(vcetype)'" != "" & "`mostopts'"!="stdp" {
			di as err "`mostopts' not available with robust VCEs"
			exit 198
		}
	}
...
"`nscores' new variables, 1 for each endogenous regressor"
	...
	foreach y of local endog {
		...
		qui `vv' _regress `y' `allinst' `wexp' if `esample', `nocons'

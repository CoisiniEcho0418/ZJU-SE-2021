...
{viewerjumpto "Saved results" "ivtobit##saved_results"}{...}
{viewerjumpto "Reference" "ivtobit##reference"}{...}
{title:Title}

{p2colset 5 20 22 2}{...}
{p2col :{manlink R ivtobit} {hline 2}}Tobit model with continuous endogenous regressors{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab :Model}
{p2coldent :* {cmd:ll}[{cmd:(}{it:#}{cmd:)}]}lower limit for left
censoring{p_end}
{p2coldent :* {cmd:ul}[{cmd:(}{it:#}{cmd:)}]}upper limit for right
censoring{p_end}
...
{p2coldent :* {opt ll}[{cmd:(}{it:#}{cmd:)}]}lower limit for left 
censoring{p_end}
{p2coldent :* {opt ul}[{cmd:(}{it:#}{cmd:)}]}upper limit for right
censoring{p_end}
...
{pstd}
{cmd:ivtobit} fits tobit models where one or more of the regressors is
endogenously determined.  By default, {opt ivtobit} uses maximum likelihood
estimation.  Alternatively, Newey's
({help ivtobit##N1987:1987}) minimum chi-squared estimator can
be invoked with the {opt twostep} option.  Both estimators assume that
the endogenous regressors are continuous and so are not appropriate for use 
with discrete endogenous regressors.
See {manhelp ivprobit R} for probit
estimation with endogenous regressors and {helpb tobit:[R] tobit} for tobit
...
{phang} 
{cmd:ll(}{it:#}{cmd:)} and {cmd:ul(}{it:#}{cmd:)} indicate the lower and upper
...
{phang}
{opt first} requests that the parameters for the reduced-form equations
showing the relationship between the endogenous variables and
instruments be displayed.  For the two-step estimator, {opt first} shows
the first-stage regressions.  For the maximum likelihood estimator,
these parameters are estimated jointly with the parameters of the tobit
equation.  The default is to not show these parameter estimates.
...
{pstd}
{cmd:ivtobit, mle} saves the following in {cmd:e()}:
...
{pstd}
{cmd:ivtobit, twostep} saves the following in {cmd:e()}:

...
program define lloghet_gp, sort
	version 7, missing
...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		tempname cc
		scalar `cc' = (exp(((0.5)^(-`theta')-1)/`theta')-1)^`gamma'
		gen `vtyp' `varn'=`cc'*exp(`xb') if `touse'
...
*/ "(option log median time assumed; predicted median log time)"
		...
		qui _predict double `xb' if `touse', xb `offset'
		tempname cc
		scalar `cc' = exp(((0.5)^(-`theta')-1)/`theta')-1 
		gen `vtyp' `varn'= `gamma'*ln(`cc')+`xb' if `touse'
...
*/ "proportional-hazards parameterization"
	...
	if "`type'"=="hazard" {
		tempvar xb ff kk
		qui _predict double `xb' if `touse', xb `offset'
		qui gen double `kk' = (`t'*exp(-`xb'))^(1/`gamma') if `touse'
		qui gen double `ff'= /*
		*/ `kk'/(`gamma'*`t'*(1+`kk')) if `touse'
		qui replace `ff'=`ff'/(1+`theta'*ln(1+`kk')) if `touse'
	...
	if "`type'" =="surv" {
		tempvar xb ff ff0
		qui _predict double `xb' if `touse', xb `offset'
		qui gen double `ff'= 1 + (`t'*exp(-`xb'))^(1/`gamma') /*
			*/ if `touse'
		qui gen double `ff0'= 1 + (`t0'*exp(-`xb'))^(1/`gamma') /*
			*/ if `t0'>0 & `touse'
	...
	if "`type'"=="mgale" { 
		...
		qui gen double `ff'= /*
		*/ (`d'!=0)-(1/`theta')* /*
		*/ ln(1+`theta'*ln(1+(`t'*exp(-`xb'))^(1/`gamma'))) /*
		*/ if `touse'
		qui replace /*
		*/ `ff'=`ff'+(1/`theta')* /*
		*/ ln(1+`theta'*ln(1+(`t0'*exp(-`xb'))^(1/`gamma'))) /*
		*/ if `t0'>0 & `touse'
		gen `vtyp' `varn' = `ff' if `touse' 
		if "`id'"!="" { local part "partial " }

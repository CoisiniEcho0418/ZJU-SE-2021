*! version 1.4.3  02feb2009
program define llogistichet, eclass byable(recall) sort prop(ml_score)
	version 7
	if replay() {
		if `"`e(cmd)'"' != "llogistichet" { error 301 } 
		if _by() { error 190 }
		Display `0'
		error `e(rc)'
		exit
...
di as error "only iweights are allowed with shared()"
	...
	if `"`from'"'=="" & "`lrtest'"=="" {
		...
		*/ "`weight'"=="pweight" {
		...
		`nlog' di as txt _n "Fitting llogistic model:"

		qui llogistic `t' `rhs' `w' if `touse', nocoef /*
	...
	ml model `mlmethod' `mlprog' /*
		...
		*/ /ln_gam /ln_the `w' if `touse', `cont' noout /*
		*/ `robust' `cluopt' `scopt' `iniopt' `mlopts' /*
		*/ missing collin nopreserve /*
		*/ max search(quietly) `log' `options' /*
		*/ diparm(ln_gam, exp label(gamma)) /*
		*/ diparm(ln_the, exp label(theta)) `negh'
	...
	est scalar gamma = exp([ln_gam]_b[_cons])        
	...
	est local cmd llogistichet 

	global S_E_cmd llogistichet

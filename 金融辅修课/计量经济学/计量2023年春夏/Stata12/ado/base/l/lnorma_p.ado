*! version 6.2.1  17jun2009
program define lnorma_p, sort
	version 7, missing
...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="time" & "`mean'"=="mean" {
		...
		gen `vtyp' `varn'=exp(`xb' + ((`sigma')^2)/2) if `touse'
...
*/ "proportional-hazards parameterization"
	...
	if "`type'"=="hazard" {
		...
		qui _predict double `xb' if `touse', xb `offset' eq(#1)
		qui gen double `ff'= /*
		*/ normd((ln(`t')-`xb')/`sigma')/(`t'*`sigma') if `touse'
		qui replace `ff'= /*
		   */ `ff'/( 1-normprob((ln(`t')-`xb')/`sigma')) if `touse'
	...
	if "`type'" =="surv" {
		tempvar xb ff
		qui _predict double `xb' if `touse', xb `offset' eq(#1)
		qui gen double `ff'= 1-normprob((ln(`t')-`xb')/`sigma') /*
		*/ if `touse'
		qui replace `ff'= /*
		   */ `ff'/( 1-normprob((ln(`t0')-`xb')/`sigma')) /*
		   */ if `t0'>0 & `touse'
		gen `vtyp' `varn' = `ff' if `touse'
		if "`t0'"=="0" { 
			label var `varn' "S(`t')"
	...
	if "`type'"=="csnell" {
		tempvar mg
		qui lnorma_p double `mg' if `touse', mgale `offset'
	...
	if "`type'"=="mgale" { 
		...
		qui _predict double `xb' if `touse', xb `offset' eq(#1)
		qui gen double `ff'= /*
		*/ (`d'!=0)+ln(1-normprob((ln(`t')-`xb')/`sigma')) if `touse'
		qui replace /*
		*/ `ff'=`ff'-ln(1-normprob((ln(`t0')-`xb')/`sigma')) /*
		*/ if `t0'>0 & `touse'
		gen `vtyp' `varn' = `ff' if `touse' 
		if "`id'"!="" { local part "partial " }
	...
	if "`is_st'"=="yes" {
		if "`type'"=="deviance" {
			tempvar cmg 
			qui lnorma_p double `cmg' if `touse', cmgale `offset' /*
				*/ `oos'
		...
		if "`type'"=="csurv" {
			if "`_dta[st_id]'" == "" {
				lnorma_p `vtyp' `varn' if `touse' & `es', /*
				*/ surv `offset' 
				exit
			}
			tempvar surv
			qui lnorma_p double `surv' if `es', surv `offset'
		...
		if "`type'"=="ccsnell" {
			...
			tempvar cs
			qui lnorma_p double `cs' if `es', cs `offset'
			sort `es' `_dta[st_id]' `t'
			qui by `es' `_dta[st_id]': replace /*
		...
		if "`type'"=="cmgale" {
			...
			tempvar mg
			qui lnorma_p double `mg' if `es', mg `offset'
			sort `es' `_dta[st_id]' `t'
			qui by `es' `_dta[st_id]': replace /*

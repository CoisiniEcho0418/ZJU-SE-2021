...
program define lnormhet_gp, sort
	version 7, missing
...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		tempname cc
		scalar `cc' = invnorm(1-exp((1-(0.5)^(-`theta'))/`theta'))
...
*/ "proportional-hazards parameterization"
	...
	if "`type'"=="hazard" {
		...
		*/ normd(`kk')/(`t'*`sigma') if `touse'
		...
		qui replace `ff'=`ff'/(1-`theta'*ln((1-norm(`kk')))) /*
		   */ if `touse'
	...
	if "`type'"=="mgale" { 
		...
		qui gen double `ff'= /*
		*/ (`d'!=0)-(1/`theta')* /*
		*/ ln(1-`theta'*ln(1-norm((ln(`t')-`xb')/`sigma'))) /*
		*/ if `touse'
		qui replace /*
		*/ `ff'=`ff'+(1/`theta')* /*
		*/ ln(1-`theta'*ln(1-norm((ln(`t0')-`xb')/`sigma'))) /*
		*/ if `t0'>0 & `touse'
		gen `vtyp' `varn' = `ff' if `touse' 
		if "`id'"!="" { local part "partial " }

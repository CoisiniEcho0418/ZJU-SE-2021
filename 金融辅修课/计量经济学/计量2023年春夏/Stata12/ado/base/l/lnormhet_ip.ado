...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		tempname cc
		scalar `cc' = invnorm(1-exp(0.5/`theta'*(1-(1-`theta'* /*
			*/ ln(0.5))^2)))
...
*/ "proportional-hazards parameterization"
	...
	if "`type'"=="hazard" {
		...
		*/ normd(`kk')/(`t'*`sigma') if `touse'
		...
		qui replace `ff'=`ff'/sqrt(1-2*`theta'*ln(1-norm(`kk'))) /*
		   */ if `touse'
	...
	if "`type'"=="mgale" { 
		...
		*/ (`d'!=0)+(1/`theta')* /*
		*/ (1-sqrt(1-2*`theta'* /*
		*/ ln(1-norm((ln(`t')-`xb')/`sigma')))) if `touse'
		qui replace /*
		*/ `ff'=`ff'-(1/`theta')* /*
		*/ (1-sqrt(1-2*`theta'*ln(1-norm((ln(`t0')-`xb') /*
		*/ /`sigma')))) if `t0'>0 & `touse'
		gen `vtyp' `varn' = `ff' if `touse' 
		if "`id'"!="" { local part "partial " }

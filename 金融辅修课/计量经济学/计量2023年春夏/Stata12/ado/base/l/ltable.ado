...
program define ltable, sortpreserve
	...
	if `"`plot'"' != "" & `"`addplot'"' != "" {
		display as error		///
		    "options addplot() and plot() may not be combined"
		exit 198
	}
	if "`graph'`overlay'" == "" {
            capture noisily 			///
	    ...
	    if _rc { 
            ...
            if "`overlay'" != "" {
                ...
                if "`by'" == "" {
                    display as error		///
                        "option overlay may not be specified without by()"
                    exit 198
                }
                if "`byopts'" != "" {
                    display as error		///
                        "options overlay and byopts() may not be combined"
                    exit 198
                }
            }
            if "`by'" == "" & `"`byopts'"' != "" {
                    display as error		///
                        "option byopts() may not be specified without by()"
                    exit 198
            }
            if "`hazard'" != "" {	
                display as error "graph not allowed with hazard"
                display as error "use sts graph to graph hazard estimates"
                exit 198
            }
	...
	if `"`graph'"'!="" {
		...
		if `"`failure'"'!="" { 
			...
			if `"`ci'"'!="" & `"`conf'"' != "noconf" {
				...
				local cigraph				///
				(rspike `upF' `loF' `x' \`ifcat',	///
					sort				///
					pstyle(ci)			///
					`ciopts'			///
					\`cicat' /// not defined yet
				)					///
				// blank
			}
		}
		...
		if `"`plot'`addplot'"' == "" {
			...
			syntax [, noDRAW * ]
			if "`draw'" != "nodraw" {
				// force first graph command not to draw
				local draw1 nodraw
			}
			else {
			   if `"`addplot'"' != "" {
				display as error		///
				  "options nodraw and addplot() may not be combined"
				exit 198
			   }
			   if `"`plot'"' != "" {
				display as error		///
				  "options nodraw and plot() may not be combined"
				exit 198
			   }
			}
		}
		...
		if "`overlay'" == "" {
			...
			forvalues i = 1/`nby' {
				...
				if `"`cigraph'"' != "" {
					local ifcat if `byid'==`i'
					// **** parse out ci`i'opts	
					local 0 , `options'
					syntax [, CI`i'opts(string asis) * ]
					local cicat `ci`i'opts'
					while `"`ci`i'opts'"' != "" {
						local 0 `", `options'"'
						syntax [, CI`i'opts(string asis) * ]
						if `"`ci`i'opts'"' != "" {
						   local cicat `"`cicat' `ci`i'opts'"'
						}
					}
					// ****
				}

...
{p2colset 5 20 22 2}{...}
{p2col :{manlink ST ltable} {hline 2}}Life tables for survival data{p_end}
{p2colreset}{...}
...
for equivalence of groups.  {cmd:ltable} also allows you to examine the
empirical hazard function through aggregation.  See also {manhelp sts ST} for
...
{it:timevar} or {it:deadvar} equal to missing are ignored.

{pstd}
{it:deadvar} does not specify the {it:number} of failures.
An observation with {it:deadvar} equal to 1 or 50 has the same 
interpretation -- the observation records one failure.  Specify frequency 
weights for aggregated data (for example, {cmd:ltable time [freq=number]}).
...
{marker noadjust}{...}
{phang}
{opt noadjust} suppresses the actuarial adjustment for deaths and censored
observations.  The default is to consider the adjusted number at risk at the
start of the interval to be total at the start minus (the number dead or
censored)/2.  If {opt noadjust} is specified, the number at risk is simply the

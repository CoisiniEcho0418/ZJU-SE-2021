*! version 2.6.5  22sep2004
program define ltable_7
	version 6, missing
	local baopts 					/*
		*/ noAdjust BY(varname) noCONF 		/*
		*/ Failure Hazard Intervals(string) 	/*
		*/ Level(integer $S_level) 		/*
		*/ SUrvival noTAb Test TVid(varname)
	local gropts 					/*
		*/ Graph Connect(string) SAving(string) /*
		*/ Symbol(string) XLAbel(string) YLAbel(string) *
...
/*------------------------- GRAPH -----------------------------------*/
		if `"`graph'"'!="" { 
			...
			if `"`xlabel'"'=="" { local xlabel xlab }
			else	local xlabel `"xlab(`xlabel')"'
			if `"`ylabel'"'=="" { local ylabel ylab } 
			else	local ylabel `"ylab(`ylabel')"'

			if `"`failure'"'!="" { 
				label var `F' "Proportion Failed"
				if `"`conf'"'=="" {
					noisily gr7 `F' `upF' `loF' `I1',/*
					*/ by(`g') `connect' s(`symbol'ii) /*
					*/ `xlabel' `ylabel' `saving' `options'
				}
				else {
					noisily gr7 `F' `I1',/*
					*/ by(`g') `connect' s(`symbol') /*
					*/ `xlabel' `ylabel' `saving' `options'
				}
			}
			else if `"`surviva'"'!="" {
				label var `S' "Proportion Surviving"
				if `"`conf'"'=="" {
					noisily gr7 `S' `upS' `loS' `I1',/*
					*/ by(`g') `connect' s(`symbol'ii) /*
					*/ `xlabel' `ylabel' `saving' `options'
				}
				else {

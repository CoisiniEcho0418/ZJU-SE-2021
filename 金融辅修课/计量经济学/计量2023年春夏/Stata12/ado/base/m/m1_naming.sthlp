...
{phang}
{manlink M-1 naming} {hline 2} Advice on naming functions and variables
...
{p 4 4 2}
Your best defense against new official functions, and other user-written
functions, is to choose long function names.
...
functions.  User-written functions are invariably library functions (here 
we draw no distinction between functions supplied in {cmd:.mo} files and those 
supplied in {cmd:.mlib} files).
...
{p 4 4 2}
A similar result is obtained if you name your function after a user-written
function that is installed on your computer.  You can do so if the
user-written function is not currently in memory.  In the future, however, one
or the other function will take precedence and, no matter which, something
will break.

...
{viewerjumpto "Saved results" "margins##saved_results"}{...}
{title:Title}

{p2colset 4 16 18 2}{...}
{p2col:{manlink R margins}}{hline 2} Marginal means, predictive margins, and
marginal effects
{p_end}
...
{synopt:{opth mcomp:are(margins##method:method)}}multiple
...
{pstd}
{cmd:fweight}s, {cmd:aweight}s, {cmd:iweight}s, {cmd:pweight}s are allowed; see
{help weight}.
...
{phang}
{bf:Statistics > Postestimation > Marginal means and predictive margins}

{phang}
{bf:Statistics > Postestimation > Marginal effects}
...
{it:Skip to} 
{bf:{mansection R marginsRemarks:Remarks}} in {manlink R margins}
{it:if you are}
...
{phang2}
    ...
    See {it:Description} and {it:Remarks} in {manlink R predictnl} for a
    ...
    {it:{mansection R marginsRemarksExample12Marginsofaspecifiedexpression:Example 12: Margins of a specified expression}} in {manlink R margins}.
...
{opt eydx(varlist)} 
    ...
    as elasticities; see 
{it:{mansection R marginsRemarksExpressingderivativesaselasticities:Expressing derivatives as elasticities}} in {manlink R margins}.
...
{opt continuous}
    ...
    as continuous; see
{it:{mansection R marginsRemarksDerivativesversusdiscretedifferences:Derivatives versus discrete differences}} in {manlink R margins}.
...
{opt asbalanced}
    ...
    see 
    {it:{mansection R marginsRemarksObtainingmarginsasthoughthedatawerebalanced:Obtaining margins as though the data were balanced}} in {manlink R margins}.
...
{opth over(varlist)} specifies that separate sets of margins be estimated
    for the groups defined by {it:varlist}. 
    The variables in {it:varlist} must contain nonnegative integer values.
    The variables need not be covariates in your model.
    When {cmd:over()} is combined with the {cmd:vce(unconditional)} option, each
    group is treated as a subpopulation;
...
{cmd:subpop(}[{varname}] [{it:{help if}}]{cmd:)} 
    ...
    or by both.  Zero indicates that the observation be excluded; 
    nonzero, that it be included; 
    and missing value, that it be treated as outside of the 
    population (and so ignored).  See 
    {manlink SVY subpopulation estimation} for why {cmd:subpop()}
...
{opt within(varlist)} 
    ...
    See 
    {it:{mansection R marginsRemarksObtainingmarginswithnesteddesigns:Obtaining margins with nested designs}} in {manlink R margins}.
...
{phang2}
    ...
    {it:{mansection R marginsRemarksObtainingmarginswithsurveydataandrepresentativesamples:Obtaining margins with survey data and representative samples}} in {manlink R margins}.

{phang} 
{opt nose}
     suppresses calculation of the VCE and standard errors.
     See 
{it:{mansection R marginsRemarksRequirementsformodelspecification:Requirements for model specification}} in {manlink R margins}
...
{opt noesample} 
    ...
    See 
{it:{mansection R marginsRemarksExample15Marginsevaluatedoutofsample:Example 15: Margins evaluated out of sample}} in {manlink R margins}.
...
{it:{mansection R marginsRemarksEstimabilityofmargins:Estimability of margins}} in {manlink R margins}.
...
{it:{mansection R marginsRemarksUsingmarginsaftertheestimatesusecommand:Using margins after the estimates use command}} in {manlink R margins}.
...
{pmore}
    Specify {cmd:nochainrule} after estimation by a user-written command.
    We recommend using {cmd:nochainrule}, even though {cmd:chainrule} is usually
    safe and is always faster.  {cmd:nochainrule} is safer because it makes no
    assumptions about how the parameters and covariates join to form the
    response. 
...
combinations.  See
{it:{mansection R marginsRemarksExample10Testingmargins---contrastsofmargins:Example 10: Testing margins -- contrasts of margins}} in {manlink R margins}.
...
{phang2}{cmd:. margins sex, predict(outcome(3))}{p_end}
...
{phang2}{cmd:. margins sex, predict(equation(distance))}{p_end}
...
{pstd}
{cmd:margins} saves the following in {cmd:r()}:
...
{synopt:{cmd:r(predict_label)}}label from {cmd:predict()}{p_end}
...
{synopt:{cmd:r(chainrule)}}chainrule information from the fitted model{p_end}
...
{pstd}
{cmd:margins} with the {cmd:post} option also saves the following in {cmd:e()}:
...
{synopt:{cmd:e(predict_label)}}label from {cmd:predict()}{p_end}
...
{synopt:{cmd:e(mcmethod)}}{it:method} from {opt mcompare()}{p_end}
{synopt:{cmd:e(mcadjustall)}}{opt adjustall} or empty{p_end}
...
{synopt:{cmd:e(chainrule)}}chainrule information from the fitted model{p_end}
{p2colreset}{...}

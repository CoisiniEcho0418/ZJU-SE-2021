...
{p2colset 5 24 26 2}{...}
{p2col :{manlink R marginsplot} {hline 2}}Graph results from margins 
...
{bf:Statistics > Postestimation > }
      {bf:Margins plots and profile plots}
...
{pmore}
	...
	{it:{mansection R marginsplotRemarksControllingthegraphsdimensions:Controlling the graph's dimensions}} in {bf:[R] marginsplot}.
...
{cmd:bydimension(}{help marginsplot##dimlist:{it:dimlist}} [{cmd:,} {help marginsplot##dimopts:{it:dimopts}}]{cmd:)} specifies
        ...
        see {it:{mansection R marginsplotRemarksThree-wayinteractions:Three-way interactions}} in {bf:[R] marginsplot}.
...
{pmore}
	...
	{it:{mansection R marginsplotRemarksHorizontalissometimesbetter:Horizontal is sometimes better}} in {bf:[R] marginsplot}.
...
{pstd}
With the exception of {opt allxlabels}, all these options may be specified
...
{pmore}
        Sometimes value labels are universally descriptive, and sometimes they
        have meaning only when considered in relation to their variable.  For
        example, "Male" and "Female" are typically universal, regardless of the
        variable from which they are taken.  "High" and "Low" may not have
        meaning unless you know they are in relation to a specific measure,
        say, blood-pressure level.  The {opt allsimplelabels} and 
...
{pmore} 
	...
	{it:{mansection R marginsplotRemarksContinuouscovariates:Continuous covariates}} in {bf:[R] marginsplot}.
...
{pmore}
	...
	{it:{mansection R marginsplotRemarksPairwisecomparisons:Pairwise comparisons}} and
	{it:{mansection R marginsplotRemarksHorizontalissometimesbetter:Horizontal is sometimes better}} in {bf:[R] marginsplot}.
...
{pmore} 
	...
	{it:{mansection R marginsplotRemarksAddingscatterplotsofthedata:Adding scatterplots of the data}} of {bf:[R] marginsplot}.
...
{it:{mansection R marginsplotRemarks:Remarks}} in {bf:[R] marginsplot}.
...
{synopt:{cmd:_deriv}}If 
	the preceding {cmd:margins} command included a {cmd:dydx()},
        ...
        also contain {cmd:_deriv} to specify all of the variables over which
...
{synopt:{cmd:_pw}}enumerates 
	all of the pairwise comparisons{p_end}

...
{viewerjumpto "Saved results" "matrix_accum##saved_results"}{...}
{title:Title}

{p2colset 5 25 27 2}{...}
{p2col :{manlink P matrix accum} {hline 2}}Form cross-product matrices{p_end}
{p2colreset}{...}
...
{phang}
{cmd:group(}{it:{help varlist:groupvar}}{cmd:)} is required with {cmd:glsaccum}
and {cmd:opaccum} and is not allowed otherwise.  In the two cases where it is
...
the name of a string variable in the dataset that contains the name of the
matrix that is to be used to weight the observations in {cmd:group()}.
...
{cmd:matrix vecaccum} save the number of observations in {cmd:r(N)}.

...
{viewerjumpto "Saved results" "mdslong##saved_results"}{...}
{viewerjumpto "References" "mdslong##references"}{...}
{title:Title}

{p 4 22 2}
{manlink MV mdslong} {hline 2} Multidimensional scaling of proximity data in long format
...
{p 8 24 2}
{cmd:mdslong} {depvar} {ifin} {weight}
...
{synopt:{cmd:s2d(}{cmdab:st:andard}{cmd:)}}convert similarity to
	dissimilarity: dissim(ij) = sqrt(sim(ii)+sim(jj)-2sim(ij)); the
	default{p_end}
...
({help mdslong##T1952:Torgerson 1952})
as well as modern metric and nonmetric MDS; see the
{helpb mdslong##method():method()},
{helpb mdslong##loss():loss()}, and
{helpb mdslong##transform():transform()} options.
...
{it:#} is 2 and should not exceed the number of positive eigenvalues of the
...
{pstd} {cmd:mdslong} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(unique)}}1 if eigenvalues are distinct, 0 otherwise{p_end}

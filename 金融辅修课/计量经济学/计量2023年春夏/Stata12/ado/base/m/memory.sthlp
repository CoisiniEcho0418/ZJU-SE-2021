...
{viewerjumpto "Saved results" "memory##saved_results"}{...}
{title:Title}

{p2colset 5 19 21 2}{...}
{p2col :{manlink D memory} {hline 2}}Memory management{p_end}
{p2colreset}{...}
...
default unit is {cmd:b}.
	   ...
	   {c |} {cmd:maxvar}      {c |}   {cmd:5000}  {c |}          {cmd:2048}          {cmd:32767}  {c |} (MP and SE)
	   {c |}             {c |}   {cmd:2048}  {c |}          {cmd:2048}           {cmd:2048}  {c |} (IC)
	   {c |}             {c |}     {cmd:99}  {c |}            {cmd:99}             {cmd:99}  {c |} (Small)
	   {c |} {cmd:}            {c |}         {c |}                               {c |}
	   ...
	   {c |} {cmd:}            {c |}     {cmd:16m} {c |}             {cmd:1m}             {cmd:1g} {c |} (32-bit)
...
{cmd:maxvar}, which is 2,048 by default. 
{cmd:set maxvar} is allowed with Stata/SE and Stata/MP only. 
In Stata/IC, {cmd:maxvar} is fixed at 2,048.  In Small Stata,
{cmd:maxvar} is fixed at 99.
...
{p 6 10 2}
3.
The maximum number of observations is fixed at 2,147,483,647 regardless 
of computer size or memory settings.  Depending on the amount of memory 
on your computer, you may face a lower practical limit.
...
16,777,216 terabytes, 16,384 petabytes, 16 exabytes.  Real computers have
less.

{p 6 10 2}
8. 
32-bit computers can theoretically provide up to 
4,294,967,296 bytes of memory, equivalent to 
4,194,304 kilobytes, 4,096 megabytes, or 4 gigabytes.  
Most 32-bit operating systems limit Stata to half that.
...
{cmd:niceness} so that Stata users cannot change them.  You may want to do this
on shared computers to prevent individual users from hogging resources.

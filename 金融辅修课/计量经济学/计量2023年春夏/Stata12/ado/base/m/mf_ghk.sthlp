...
{p 4 21 2}
{manlink M-5 ghk()} {hline 2} Geweke-Hajivassiliou-Keane (GHK) multivariate normal simulator
...
{p 4 4 2}
{it:S} {cmd:= }{cmd:ghk_init(}{it:npts}{cmd:)} constructs the transmorphic
object {it:S}.  Calls to 
...
differencing (for the Newton-Raphson optimize technique) when few simulation
points are used, resulting in a non-positive-definite Hessian.  To turn off the
interval pivoting, call {cmd:ghk_init_pivot(}{it:S}{cmd:, }0{cmd:)} prior to
executing {cmd:ghk()}.
...
transmorphic object {it:S} is not recreated on each call to the likelihood
evaluator.

...
{p 4 8 2}
{manlink M-5 tokenget()} {hline 2} Advanced parsing
...
{it:wchars} is a {it:string scalar} containing the characters to be 
    treated as white space, such as 
...
white-space characters {it:wchars}, (2) the parsing characters {it:pchars},
...
white-space characters, no parsing characters, and no quote characters, 
...
{p 12 16 2}
a.  {it:wchars} is a {it:string scalar}.  The white-space characters 
    appear one after the other in the string.  The order in which the 
    characters appear is irrelevant.

{p 12 16 2}
b.  Specify {it:wchars} as {cmd:" "} to treat blank as white space.

{p 12 16 2}
c.  Specify {it:wchars} as {cmd:" "+char(9)} to treat blank and {it:tab}
    as white space.  Including {it:tab} is necessary only when strings
    to be parsed are obtained from a file; strings obtained from Stata 
    already have the {it:tab} characters removed.

{p 12 16 2}
d.  Any character can be treated as a white-space character, 
    including letters.

{p 12 16 2}
e.  Specify {it:wchars} as {cmd:""} to specify that there are no white-space
    characters.
...
If the line was empty or contained only white-space characters, 

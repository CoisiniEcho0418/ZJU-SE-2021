*! version 1.0.0  08jun2011
program define mgarch_vcc, sortpreserve eclass byable(recall)
	...
	syntax anything [if] [in],                       ///
		...
		noLOg					 ///
		DISTribution(string)			 ///
		PSIlag(numlist >0 integer max=1)	 ///  /* UNDOCUMENTED */
		*					 ///
		]
	...
	// returns a structure which is in turn passed to MGARCH_main_VCC()
		...
		forvalues j=1/`howmany' {
			...
			if "`s`k'_arch'" != "" {
				foreach i of numlist `s`k'_arch' {
				    local STRIPE `STRIPE' ARCH_`dv':L`i'.arch
				}
...
exit 498
				...
				foreach i of numlist `s`k'_garch' {
				    local STRIPE `STRIPE' ARCH_`dv':L`i'.garch
				}
	...
	if `k_eq'>1 {
		...
		forvalues i = 1/`r' {
			...
			forvalues j = `c'/`k_eq' {
				...
				local STRIPE `STRIPE' corr(`dep1',`dep2'):_cons
			}
		}
		local STRIPE `STRIPE' Adjustment:lambda1 Adjustment:lambda2
	}
	
	if (`estdf') local STRIPE `STRIPE' `name':_cons
	...
	mata: MGARCH_main_VCC(`k_eq',"`touse'",`T',`mlopts',"`tvar'",`isrobust')
	...
	ereturn scalar psilag = `psilag'
	ereturn scalar converged = `converged'
	ereturn scalar ic = `ic'
	ereturn scalar rc = `rc'
	
	// macros
	ereturn local estat_cmd "_mgarch_vcc_estat"
	ereturn local predict "mgarch_vcc_p"
	...
	ereturn local model "vcc"
	ereturn local cmd "mgarch"
	ereturn local title "Varying conditional correlation MGARCH model"

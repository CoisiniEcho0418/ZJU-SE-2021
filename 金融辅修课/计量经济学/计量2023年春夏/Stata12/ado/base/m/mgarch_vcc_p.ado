...
program define mgarch_vcc_p, properties(notxb)
	version 11
	...
	if "`e(cmd)'" != "mgarch" | "`e(model)'" != "vcc" {
		exit 301
	}
	
	opts_exclusive "`xb' `residuals' `variance'"
	...
	if `"`dynamic'"' == "" {
		markout `touse' `e(depvar)' `e(covariates)'
	}
	else {
	...
	if `"`dynamic'"' == "" {
		...
		if "`stat'"=="xb" {
			...
			foreach v in `newvarlist' {
				...
				local lab   : word `i' of `lablist'
				qui gen `type' `v' = .
				label variable `v' "Variance prediction `lab'"
				local ++i
			}
			
			mata: MGARCH_predict_VCC("`e(hetvars)'", 	  ///
				"`e(arch)'","`e(garch)'","`newvarlist'",   ///
				"`reslist'","`e(tvar)'","`touse'","`duse'", ///
				`ix1',`ix2')
			exit
		}
		else {
		...
		if "`stat'" == "residuals" {
			...
			mgarch_vcc_p double (`newlist') if `touse', xb
...
local nname : subinstr local nname "`old'" "`new'", all word
			...
			foreach v in `newvarlist' {
				...
				local lab   : word `i' of `lablist'
				qui gen `type' `v' = .
				label var `v' "Variance prediction `lab'`dpart'"
				local ++i
			}

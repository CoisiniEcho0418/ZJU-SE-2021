/* -------------------------------------------------------------------- */
*! version 1.1.0  03jul2009

/*
	mi passive [, noupdate nopreserve]: [<by>] <cmd>
...
void mi_passive_split_varlist(`SS' left, `SS' right, `SS' s)
{
	...
	l = strpos(s, "(")
	if (l) { 
		st_local(left,  substr(s, 1, l-1))
		st_local(right, substr(s, l, .))
	}
	else {
		st_local(left,  s)
		st_local(right, "")
...
program mi_passive_parse_cmd
	...
	if ("`gencmd'"==substr("generate", 1, max(1,`l'))) {

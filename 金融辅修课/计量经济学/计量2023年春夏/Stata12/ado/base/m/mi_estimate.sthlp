...
{viewerjumpto "Saved results" "mi_estimate##saved_results"}{...}
{viewerjumpto "References" "mi_estimate##references"}{...}
{title:Title}

{p2colset 5 25 27 2}{...}
{p2col:{manlink MI mi estimate} {hline 2}}Estimation using multiple imputations{p_end}
{p2colreset}{...}
...
{syntab:Tables}
{synopt:[{cmdab:no:}]{cmdab:citab:le}}suppress/display the standard estimation
...
{synopt:{it:{help mi_estimate##xtme_options:xtme_options}}}control output from mixed-effects commands{p_end}
...
even if they were previously computed.
...
commands that account for small samples.  If the command saves residual degrees
...
the command does not save residual degrees of freedom, the large-sample test is
used and the {cmd:nosmall} option has no effect.

{phang}
{opt saving(miestfile [, replace])} saves estimation results
...
in {manhelp mi_estimation MI:mi estimation}.
...
{opt nofet:able}, and
{opt estm:etric}.
These options are relevant only with
the mixed-effects commands such as {cmd:xtmixed} (see {manhelp xtmixed XT}),
{cmd:xtmelogit} (see {manhelp xtmelogit XT}), and {cmd:xtmepoisson} (see
{manhelp xtmepoisson XT}).  The {cmd:estmetric} option is implied when
{cmd:vartable} or {cmd:dftable} is used.
...
(see {mansection MI miestimateRemarksPotentialproblemsthatcanarisewhenusingmiestimate:{it:Potential problems that can arise when using mi estimate}}
...
{mansection MI miestimateRemarksPotentialproblemsthatcanarisewhenusingmiestimate:{it:Potential problems that can arise when using mi estimate}}
...
{mansection MI miestimatepostestimationRemarksUsingthecommand-specificpostestimationtools:{it:Using the command-specific postestimation tools}} in
...
{pstd}
Estimate on completed data using {cmd:xtmixed}
{p_end}
{phang2}
{cmd:. webuse mjsps5}
{p_end}
{phang2}
{cmd:. mi estimate, dots: xtmixed math5 math3 || school:, reml}
{p_end}
...
{pstd}
{cmd:mi estimate} saves the following in {cmd:e()}:

{synoptset 22 tabbed}{...}
{p2col 5 15 19 2: Scalars}{p_end}
{synopt:{cmd:e(df_avg}[{cmd:_Q}]{cmd:_mi)}}average degrees of freedom{p_end}
{synopt:{cmd:e(df_c_mi)}}complete degrees of freedom (if originally saved by 
...
{synopt:{cmd:e(b)}}MI estimates of coefficients (equals {cmd:e(b_mi)}, saved only if {cmd:post} is used{p_end}
{synopt:{cmd:e(V)}}variance-covariance matrix (equals {cmd:e(V_mi)}, saved only if {cmd:post} is used{p_end}
{synopt:{cmd:e(Cns)}}constraint matrix (for constrained estimation only; equals {cmd:e(Cns_mi)}, saved only if {cmd:post} is used){p_end}
...
saved for panel-data models only.  The results are saved as matrices for
...
saved with the {cmd:_Q_mi} suffix, as described above.

{phang}
Command-specific {cmd:e()} results that remain constant across imputations
are also saved.  Command-specific results that vary from imputation to
imputation are posted as missing, and their names are saved in the
...
{it:vs_names} above), their values from each imputation are saved in a

...
{viewerjumpto "Saved results" "mi_merge##saved_results"}{...}
{title:Title}

{p2colset 5 22 24 2}{...}
{p2col :{manlink MI mi merge} {hline 2}}Merge mi data{p_end}
{p2colreset}{...}
...
{p 4 4 2}
{cmd:mi merge} saves the following in {cmd:r()}:

	Scalars
	    {cmd:r(N_master)}       # of obs. in {it:m}=0 in master
	    {cmd:r(N_using)}        # of obs. in {it:m}=0 in using
	    {cmd:r(N_result)}       # of obs. in {it:m}=0 in result

	    {cmd:r(M_master)}       # of imputations ({it:M}) in master
	    {cmd:r(M_using)}        # of imputations ({it:M}) in using 
	    {cmd:r(M_result)}       # of imputations ({it:M}) in result 

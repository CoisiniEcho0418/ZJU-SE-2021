...
{viewerjumpto "Saved results" "mi_xeq##saved_results"}{...}
{title:Title}

{p2colset 5 20 22 2}{...}
{p2col :{manlink MI mi xeq} {hline 2}}Execute command(s) on individual
     imputations{p_end}
{p2colreset}{...}
...
{p 4 4 2}
{cmd:mi} {cmd:xeq} 
saves in {cmd:r()} whatever the last command run on the last 
imputation or specified imputation returns.  For instance, 

	. {cmd:mi xeq: tabulate g ;  summarize x}

{p 4 4 2}
returns the saved results for {cmd:summarize} {cmd:x} run on {it:m}={it:M}.  

	. {cmd:mi xeq 1 2: tabulate g ;  summarize x}

{p 4 4 2}
returns the saved results for {cmd:summarize} {cmd:x} run on {it:m}=2.  

	. {cmd:mi xeq 0: summarize x}

{p 4 4 2}
returns the saved results for {cmd:summarize} {cmd:x} run on {it:m}=0.  
{p_end}

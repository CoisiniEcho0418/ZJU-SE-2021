...
// sample ----------------------------------------------------------------------
	...
	if "`matrix'" != "" { 
		local matsizeneeded = max(`N',`nvar')
		if `matsizeneeded' > c(matsize) {
			dis as err ///
			    "matsize too small to create a [`N',`nvar'] matrix"
			exit 908
		}
	}
	else { 
		if `N' > c(matsize) {
			dis as err ///
			    "matsize too small to create `N' x 1 column vectors"
			exit 908
		}
	}
...
// handle the stripes ----------------------------------------------------------
	...
	if "`rownames'" == "" {
		...
		if substr("`:type `rownames''",1,3) == "str" {
...
program CleanUp
	...
	qui replace `vn' = subinstr(`vn',".","_",.) if `touse'
	qui replace `vn' =   substr(`vn',1,`nchar')   if `touse'
end

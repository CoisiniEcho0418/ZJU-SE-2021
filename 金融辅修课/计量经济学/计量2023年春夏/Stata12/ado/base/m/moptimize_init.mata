	...
	if (args() == 2) {
		...
		if (M.eqcolnames[eq] == NULL) {
			if (strlen(M.st_eqlist[eq])) {
				colnames = _st_ivarlist(M.st_eqlist[eq])
				if (hascons) {
...
)
{
	...
	if (isreal(offset)) {
		...
		M.st_eqoffset[eq] = mopt__check_varnames(offset, 0,1,
			"offset")
...
scalar mopt__vcetype_str(`MoptStruct' M, |string scalar uvcetype)
{
	...
	else if (uvcetype == substr(`MOPT_vcetype3',1,max((1,len)))) {
		// Robust
		return(3)
	}
	else if (uvcetype == substr(`MOPT_vcetype4',1,max((2,len)))) {
		// CLuster
		return(4)
	}
...
function moptimize_init_svy_setup(`MoptStruct' M)
{
	...
	if (rows(w)) {
		...
		if (strlen(M.st_genwvar)) {
			...
			st_view(sub=., ., M.st_genwvar, M.st_touse)
			sub[,]	= w
		}
	}
	M.valid	= `OPT_onoff_off'

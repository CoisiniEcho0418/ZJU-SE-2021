...
{viewerjumpto "Saved results" "mvtest_means##saved_results"}{...}
{viewerjumpto "References" "mvtest_means##references"}{...}
{title:Title}

{p2colset 5 26 22 2}{...}
{p2col :{manlink MV mvtest means} {hline 2}}Multivariate tests of means{p_end}
{p2colreset}{...}
...
{p 8 16 2}
{cmd:mvtest} {cmdab:m:eans} {varlist} {ifin} {weight}{cmd:,}
...
{synopt:{opt het:erogeneous}}James' test for equal means, allowing
...
the four standard tests of multivariate means (Wilks' lambda, Pillai's trace,
...
test) saves the following in {cmd:r()}:
...
{synopt:{cmd:r(p_F)}}significance{p_end}
...
options (that is, a multiple-sample means test assuming homogeneity) saves the
following in {cmd:r()}:
...
heterogeneity) saves the following in {cmd:r()}:
...
for heterogeneity) saves the following in {cmd:r()}:
...
{synopt:{cmd:r(p_chi2_James)}}significance via James' approximation{p_end}

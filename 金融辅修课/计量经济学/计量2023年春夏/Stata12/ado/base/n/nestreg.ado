...
program define nestreg, byable(recall) rclass
	...
	quietly ssd query
	if (r(isSSD)) {
		di as err "nestreg not possible with summary statistc data"
		exit 111
	}
	// <my_stuff> : <command>
	_on_colon_parse `0'
	...
	_prefix_depvarblocklist `cmdname'	///
		: depvar termlist vlist k term	///
		: `termlist'
	...
	_prefix_varblocklist termlist vlist k term : `r(varblocklist)'
	if "`cmdname'" == "intreg" {
		markout `touse' `vlist'
	}
	else {
	...
	local varlist : subinstr local colna "_cons" "", all word
	local dropped : list vlist - varlist
	...
	if "`dropped'" != "" {
		...
		foreach dvar of local dropped {
			di as txt ///
"note: `dvar' dropped because of estimability"
		}
	}
	...
	if r(N) & ! inlist("`cmdname'",				///
			substr("logit",1,max(4,`lcmd')),	///
			substr("probit",1,max(4,`lcmd'))) {
		if "`svyopt'" == ""			///
...
"this violates the nested model assumption{p_end}"
	...
	if "`waldtable'" != "" {
		...
		local lcmd : length local cmdname
		if "`cmdname'" == substr("regress",1,max(1,`lcmd')) {
...
program CheckProps
	...
	if "`swprop'" == "" { 
		di as err "`cmdname' is not supported by nestreg"
		exit 199
	}
	if "`lrtable'" != "" {
		local swlist swml
		if `"`:list swlist & props'"' == "" {
			di as err ///
			"`cmdname' does not allow the lrtable option of nestreg"
			exit 199
		}
	}
end

...
{viewerjumpto "Saved results" "nlsur##saved_results"}{...}
{title:Title}

{p2colset 5 18 20 2}{...}
{p2col :{manlink R nlsur} {hline 2}}Estimation of nonlinear systems of equations{p_end}
{p2colreset}{...}
...
{cmd:nlsur} {it:sexp_prog} {cmd::} {it:{help depvar:depvar_1}} 
   ...
   {weight} [{cmd:,} {it:{help nlsur##options_table:options}}]
...
{synopt :{opt neq:uations(#)}}number of equations in model (function evaluator program version only){p_end}
...
{synopt :{it:{help nlsur##display_options:display_options}}}control column
...
{synopt :{opt noc:onstants}}no equations have constant terms{p_end}
{synopt :{opt h:asconstants(namelist)}}use {it:namelist} as constant terms{p_end}

INCLUDE help shortdes-coeflegend
{synoptline}
{p2colreset}{...}
{p 4 6 2}* You must specify {opt parameters(namelist)}, {opt nparameters(#)}, or both.{p_end}
...
[{ul:{cmd:no}}]{cmdab:lo:g},
{cmdab:tr:ace}.  {cmd:iterate()} specifies the maximum number of
iterations to use for NLS and at each round of FGNLS estimation.
...
whether to show the iteration log, and {opt trace} specifies 
that the iteration log should include the current parameter vector.
...
(.00001).  {opt eps()} also specifies the convergence criterion for 
...
16,000 by default.  To use this option, you must also specify the {opt ifgnls}
option.
...
{p 12 18 2}{cmd:version 12}{p_end}
...
{p 16 22 2}{cmd:version 12}{p_end}
...
{pstd}
{cmd:nlsur} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(converge)}}{cmd:1} if converged, {cmd:0} otherwise{p_end}

{p2col 5 15 17 2: Macros}{p_end}
{synopt:{cmd:e(cmd)}}{cmd:nlsur}{p_end}
{synopt:{cmd:e(cmdline)}}command as typed{p_end}
{synopt:{cmd:e(method)}}{cmd:nls}, {cmd:fgnls}, or {cmd:ifgnls}{p_end}

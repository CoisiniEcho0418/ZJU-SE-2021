...
`Errcode' opt__loop_bhhh(`OptStruct' S)
{
	...
	for (iter=1; iter <= maxiter & !S.converged; ++iter) {
		opt__dots(S)
...
/*STATIC*/ void opt__print_log(`OptStruct' S)
{
	...
	if (!S.debugeval) {
		...
		if (S.trace_step) {
			...
			else if (S.trace_gradient & S.trace_hessian) {
				opt__print_vector(
					S.gradient,
					(cols(S.gradient)==S.dim
						? *S.stripes[3]
	...
	opt__print_hessian(S.H,
		(cols(S.H)==S.dim ? *S.stripes[2] : J(0,2,"")),
		(cols(S.H)==S.dim ? *S.stripes[3] : J(0,2,"")),
		S.minimize)
}
...
function opt__tracelevel_str(`OptStruct' S, |string scalar trace)
{
	if (args() == 2) {
		real scalar len
		len = strlen(trace)
		if (trace == substr(`OPT_tracelvl0',1,max((1,len)))) {
			S.utrace = 0
		}
		else if (trace == substr(`OPT_tracelvl1',1,max((1,len)))) {
			S.utrace = 1
		}
		else if (trace == substr(`OPT_tracelvl2',1,max((2,len)))) {
			S.utrace = 2
		}
		else if (trace == substr(`OPT_tracelvl3',1,max((1,len)))) {
			S.utrace = 3
		}
		else if (trace == substr(`OPT_tracelvl4',1,max((6,len)))) {
			S.utrace = 4
		}
		else if (trace == substr(`OPT_tracelvl4alt',1,max((5,len)))) {
			S.utrace = 4
		}
		else if (trace == substr(`OPT_tracelvl5',1,max((1,len)))) {
			S.utrace = 5
		}
		else if (trace == substr(`OPT_tracelvl5alt',1,max((1,len)))) {
			S.utrace = 5
		}
		else if (trace == substr(`OPT_tracelvl6',1,max((1,len)))) {
			S.utrace = 6
		}
		else if (trace == substr(`OPT_tracelvl7',1,max((1,len)))) {
			S.utrace = 7
		}
		else {
...
`Errcode' opt__validate(`OptStruct' S, |real scalar is_mopt)
{
	...
	if (S.technique != `OPT_technique_nr') {
		...
		else if (strpos(S.evaltype_f, "??debug")) {
...
function opt__wtype_str(`OptStruct' S, |string scalar wtype)
{
	...
	else if (wtype == substr("weight",1,len)) {
		S.wtype	= `OPT_wtype_f'
	}
	else if (wtype == substr(`OPT_wtype1',1,len)) {
		S.wtype	= 1
	}
	else if (wtype == substr(`OPT_wtype2',1,len)) {
		S.wtype	= 2
	}
	else if (wtype == substr(`OPT_wtype3',1,len)) {
		S.wtype	= 3
	}
	else if (wtype == substr(`OPT_wtype4',1,len)) {
		S.wtype	= 4
	}
	else {
		errprintf("invalid weight type\n")

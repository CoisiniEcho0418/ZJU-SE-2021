...
replication.  {opt every()} should be specified only in conjunction
with {opt saving()} when {it:command} takes a long time for each replication.
This option will allow recovery of partial results should some other software
crash your computer.  See {manhelp postfile P}.

...
program probit, properties(svyb svyj svyr swml mi) eclass byable(onecall)
	if replay() {
		version 11
	...
	`version' `BY' _vce_parserun probit, mark(OFFset CLuster) : `0'
	if "`s(exit)'" != "" {
		ereturn local cmdline `"probit `0'"'
		exit
	}
...
program Estimate, eclass byable(recall)
	...
	if "`n0'" != "" {
		local lf0 = `n1'*ln(normal(`b0')) +(`n0')*ln(normal(-`b0'))
	...
	if `:length local vce' |	///
	   ...
	   `:length local score' {
	 	local evaltype e2
		local myeval mopt__probit_e2()
	}
	else {
	 	local evaltype d2
		local myeval mopt__probit_d2()
	}
...
capture noisily break {
	...
	ereturn local estat_cmd probit_estat
	ereturn local cmd probit

	Replay , `table' `header' norules `grouped' `diopts'
end

program Replay
	syntax [, notable noHeader NOCOEF noRULES GROUPED *]
	...
	if "`grouped'" != "" {
		local title title(Probit regression for grouped data)
	}

...
void setup_problem(`PROBLEM' pr, `SS' touse, `SS' in)
{
	...
	if ((in=strtrim(in)) != "") {
		l  = strpos(in, "/")
		lb = strtoreal(substr(in,   1, l-1))
		ub = strtoreal(substr(in, l+1,   .))
		pr.inrange = lb, ub
	}
	else	pr.inrange = .
}
...
void parse_putlist_name_equal(`Tr' t, `SS' name, `PROBLEM' pr)
{
	...
	contents = parse_varlist(token, 0)
...
void parse_putlist_name_equal_paren(`SS' name, `SS' list, `PROBLEM' pr)
{
	...
	evarlist = substr(list, 1, strlen(list)-1)
	contents = parse_evarlist(substr(evarlist, 2, .), 0, hasnumeric)
...
/*	________				     __________
	contents = parse_evarlist(evarlist, emptyok, hasnumeric)
				  --------  -------  
...
`SR' parse_evarlist(`SS' evarlist, `boolean' emptyok, `boolean' hasnumeric)
{
	`SR'	result, toparse
	...
	while (length(toparse)==3) {
		hasnumeric = `True'
		result = (result, parse_varlist(toparse[1], 1), toparse[2])
		toparse = split_on_number(toparse[3])
	}
	return((result, parse_varlist(toparse, 1)))
}
...
`SR' parse_varlist(`SS' varlist, `boolean' emptyok)
{
	`RS'	rc
	`SS'	result

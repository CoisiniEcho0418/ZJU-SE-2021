*! version 7.3.8  24jul2008
program define roccomp, rclass
	version 6, missing
...
program define ROCcomp, sclass         /* Dependent (correlated) ROC curves */
	...
	while `i' <= `bymax' {
		...
		*/ `lb' `ub' `D' `C' `wv' `id' `order'
		...
		if `"`binormal'"'~= "" {
			...
			local troc="0"+string(round(`area',0.0001))
			label var yhat`i' `"`group' binormal fit"'
			local yhatgr="`yhatgr' yhat`i'"
		}
	...
	if `"`graph'"' ~= `""' { /* set graph7 defaults */
		...
		local ngr : word count `sensgr'
		forval i = 1/`ngr' {
			local plopts `plopts' PLOT`i'opts LINE`i'opts
		}
		_get_gropts, graphopts(`options') getallowed(`plopts')
		local options `"`s(graphopts)'"'
		forval i = 1/`ngr' {
			local opts `"`s(plot`i'opts)'"'
			_check4gropts plot`i'opts, opt(`opts')
			local y : word `i' of `sensgr'
			local yh : word `i' of `yhatgr'
			local sgraphs `sgraphs'		///
			(connected `y' `x',		///
				sort			///
				`opts'			///
			)
			if "`yh'" != "" {
				local opts `"`s(line`i'opts)'"'
				_check4gropts line`i'opts, opt(`opts')
				local sgraphs `sgraphs'		///
				(line `yh' `x',			///
					sort			///
					`opts'			///

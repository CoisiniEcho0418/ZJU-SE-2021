*! version 7.5.0  13apr2010
prog def rocfit, eclass sort
	version 6, missing
	...
	if replay() {
		...
		*/ MLOpt(string) noLOG  CONTinuous(string) Generate(string) * ]
		...
		if "`continuous'"~="" & "`continuous'"~="." {
			...
			tempvar pct
			tempname cmax cmin diff top 	
			qui sum `C' if `touse', meanonly
			scalar `cmax'=r(max)
			scalar `cmin'=r(min)
			scalar `diff' = (`cmax'-`cmin')/`continuous'
			scalar `top' = `cmin' 
			qui gen double `pct' = .
			forvalues grp = 1/`=`continuous'-1' {
				scalar `top' = `top' + `diff'
				qui replace `pct' = `grp' if `C'<=`top' /* 
					*/  & `pct'==. & `touse'
			}
			qui replace `pct' = `continuous' if `C'>`top' & /*
				*/ `pct'==. & `touse'
			qui tab `pct'
			if `r(r)' < `continuous' {
				local numgrp = `r(r)'
				local grpnote "neednote"
			}
			tempvar C
			qui gen `C'=`pct'
			qui drop `pct'
		...
		if  r(N_unique)<. & r(N_unique)>`mymat'-3 {
			if "`continuous'"=="" {
				di in red /*
				*/ "matsize too small; type -help matsize-" 
				di in red /*
				*/ "you may want to specify continuous() option" 
				exit 908
			}
			else {
				di in red /*
				*/ "matsize needs to be at least: " /*
				*/ `r(N_unique)'+3 
				error 908
			}
                }
		...
		*noi di in red "`zv'"
		...
		local a = `B'[1,1]
		local b = `B'[1,2]
		mat colnames `B' = intercept:_cons slope:_cons `zname'
		tempname x2 df
		tempvar num
		qui gen double `num'= /*
		*/ (`MN'/$MP)*(($MP - `wv'/`MN')^2) if `touse'
		...
		est local title  = "Binormal model of "+abbrev("`D'",12)+ /*
			*/ " on " + abbrev("`ind'",12)
		est hidden local predict _no_predict
		est local depvar "`D' `DC'"
		version 10: ereturn local cmdline `"rocfit `cmdline'"'
	...
	*/ in gr ") =  " in ye %10.2f e(chi2_gf)
	...
	DISTab2 `level'
	if "`grpnote'" != "" {
		di in smcl as txt "Note: `continuous' groups were " ///
                "requested but only `numgrp' groups were created " ///
                "because data " _n _col(7) "is not evenly spaced.  " ///
                "To create groups for the classification variable " _n ///
                _col(7) "based on quantiles, you can use {helpb xtile}."
	} 
end
...
prog def DISTab1
	...
	DivLine top
	if "`e(vcetype)'" == "OPG" {
		di in smcl in gr _col(13) /*
				*/    "{c |}                 OPG"
	}
	di in smcl in gr "             {c |}      Coef.   Std. Err."  /*
*/ `"      z    P>|z|`spaces'[`=strsubdp("`level'")'% Conf. Interval]"'
	DivLine mid

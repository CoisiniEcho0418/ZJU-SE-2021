...
program Estimate, eclass sortpreserve
	...
	local modlinear = substr("linear",1,length("`ctrlmodel'"))	
	local modstrata = substr("strata",1,length("`ctrlmodel'"))	
	if "`ctrlcov'" != "" {
	...
	// check pvc
	local pvcemp = substr("empirical",1,length("`pvc'"))	
	local pvcnorm = substr("normal",1,length("`pvc'"))	
...
"are created as _fpr_`var', _roc_`var'{p_end}"
	...
	if (`"`ctrlcov'"' != "" & `"`ctrlmodel'"'==`"strata"') {
		...
		qui {
			...
			by `stratano': gen `strcaseno' = _N - `strctrlno'
		}
	...
	if (`isml') {
		...
		local inoiml inoiml
		di ""
		di as text "Parametric ROC estimation"
		di ""
		// Display first time
                if(`"`ctrlcov'"' != "") {
	                di as text ///
...
"  Covariate control adjustment model:"
				tempname curest
				qui estimates store `curest'
				qui estimates restore rr_`classvar'
				_coef_table, level(`level') `diopts'
				qui estimates restore `curest'
				qui estimates drop `curest'
			}

...
{viewerjumpto "Options for ROC using bootstrap" "rocreg##options_boot"}{...}
{viewerjumpto "Examples" "rocreg##examples"}{...}
{viewerjumpto "Saved results" "rocreg##saved_results"}{...}
{title:Title}

{p2colset 5 19 21 2}{...}
{p2col:{manlink R rocreg} {hline 2}}Receiver operating characteristic (ROC) regression{p_end}
{p2colreset}{...}
...
{it:{help rocreg##boot_options:boot_options}}]
...
{synopt:{opth cl:uster(varname)}}variables identifying resampling clusters{p_end}
...
{synoptline}
{syntab:Model}
{synopt:{opt probit}}fit the probit model{p_end}
...
{synopt:{opth cl:uster(varname)}}variables identifying resampling clusters
{p_end}
...
{syntab:Model}
{synopt:{opt probit}}fit the probit model{p_end}
{synopt:{opt ml}}fit the probit model by maximum likelihood estimation{p_end}
{synopt:{opth roccov(varlist)}}covariates affecting ROC curve
	{it:varlist}{p_end}
{synopt:{opth cl:uster(varname)}}variables identifying resampling clusters
        {p_end}
{synopt:{opth ctrlcov(varlist)}}adjust control distribution for covariates in
...
{p2coldent:* {cmd:bsave(}{it:{help filename}}{cmd:,} ...{cmd:)}}save bootstrap
   replicates from parametric estimation{p_end}
{p2coldent:* {opth bfile(filename)}}use bootstrap replicates dataset for estimation replay{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}
* {opt bsave()} and {opt bfile()} are allowed only when the {opt probit} option
is also specified. 
{p_end}
...
{phang}
{opth cluster(varname)} specifies variable identifying resampling clusters.
...
{opt tiecorrected} adjusts the percentile values for ties.  For each value of
the classifier, one half the probability that the classifier equals that value
under the control population is added to the percentile value.  Not allowed
with {cmd:pvc(normal)}.
...
used.  The {cmd:technique(bhhh)} option is not allowed.


{marker options_boot}{...}
{title:Options for rocreg, using bootstrap}
...
in estimation replay.  {cmd:bfile()} is only allowed with parametric analysis
using bootstrap. 
...
{pstd}
Nonparametric {cmd:rocreg} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(tiecorrected)}}indicates whether tie correction was used{p_end}
{synopt:{cmd:e(nobootstrap)}}indicates that bootstrap was performed{p_end}
{synopt:{cmd:e(bseed)}}seed used in bootstrap, if bootstrap was performed{p_end}
{synopt:{cmd:e(breps)}}number of bootstrap resamples, if bootstrap performed{p_end}
{synopt:{cmd:e(cc)}}indicates whether case-control groups were used as resampling strata{p_end}
{synopt:{cmd:e(nobstrata)}}indicates whether resampling should stratify based on control covariates{p_end}
...
{pstd}
Parametric, bootstrap {cmd:rocreg} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(title)}}title in estimation output{p_end}
{synopt:{cmd:e(tiecorrected)}}indicates whether tie correction was used{p_end}
{synopt:{cmd:e(probit)}}{cmd:probit}{p_end}
...
{synopt:{cmd:e(nobootstrap)}}indicates that bootstrap was performed{p_end}
{synopt:{cmd:e(bseed)}}seed used in bootstrap{p_end}
...
{pstd}
Parametric, maximum likelihood {cmd:rocreg} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(ml)}}indicates that maximum likelihood estimation was used{p_end}
{synopt:{cmd:e(predict)}}program used to implement {cmd:predict}{p_end}

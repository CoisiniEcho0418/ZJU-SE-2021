...
{viewerjumpto "Saved results" "serset##saved_results"}{...}
{title:Title}

{p2colset 5 19 21 2}{...}
{p2col :{manlink P serset} {hline 2}}Create and manipulate sersets{p_end}
{p2colreset}{...}
...
{pstd}
The following {help macro:extended macro functions} are also available:

	Extended function             Returns from the current serset
	{hline 61}
	{cmd:: serset id}                   ID
...
{pstd}
{cmd:serset create}, {cmd:serset create_xmedians}, {cmd:serset create_cspline}, {cmd:serset set}, and {cmd:serset} save the following in
{cmd:r()}:

...
{vieweralsosee "[U] 18.11.6 Writing online help" "help examplehelpfile"}{...}
{viewerjumpto "Description" "smcl##description"}{...}
{viewerjumpto "Remarks" "smcl##remarks"}{...}
{title:Title}

{p2colset 5 17 19 2}{...}
{p2col :{manlink P smcl} {hline 2}}Stata Markup and Control Language{p_end}
{p2colreset}{...}
...
help mode, is included for backward compatibility and should be avoided.  See
{mansection P smclRemarksDirectiveforenteringtheas-ismode:{it:Directive for entering the as-is mode}}
and
{mansection P smclRemarksDirectiveforenteringtheStata6helpmode:{it:Directive for entering the Stata 6 help mode}}
in {hi:[P] smcl}.
...
{pstd}
In syntaxes 3 and 4, {it:text} may contain other SMCL
...
{it:{help smcl##ascii:Displaying characters using ASCII code}} below.
...
{cmd:{c -(}}, {cmd:{c )-}}, {cmd:[}, and {cmd:]}.
    ...
    {cmd:{c -(}news:}{it:text}{cmd:{c )-}} follows {help smcl##syntax:syntax 2}.
{pin}{cmd:{c -(}news{c )-}} displays {it:text} as a link that will display
in the Viewer the latest news from http://www.stata.com.
...
a source (typically http://www.stata.com, but perhaps a local CD
...
{cmd:{c -(}c{c )-}} --  allows you to output any ASCII character.  For
...
{pstd}
Also, so that you do not have to remember the ASCII numbers,
...
{pstd}
For the Mac, however, Stata uses the Mac encoding,

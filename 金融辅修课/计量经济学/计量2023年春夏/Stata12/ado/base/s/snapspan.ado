...
program define snapspan
	...
	if r(N) { 
		...
		di in red "either fill `them' in with approprite values or" 
	...
	sort `id' `t'
	tempvar cnt
	qui by `id': gen long `cnt' = `t'==`t'[_n-1] | `t'==`t'[_n+1]
	qui count if `cnt'
	if r(N) { 
		local ndup = r(N)
	...
	tempvar obsper
	qui by `id': gen long `obsper' = _N if _n==1
...
spanfill id t varlist, fowards(.) backwards

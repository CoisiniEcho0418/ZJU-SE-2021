...
"stripe matrix in second argument has too few rows\n")
		...
		for (i=1; i<=r; i++) {
			for (j=2; j<=c0; j++) {
				if (strlen(myrstripe[i,j])) {
					lastch	= substr(myrstripe[i,j],-1,1)
					hasop	= strpos(".@#|*", lastch)
					rwidth	= max((	rwidth,
	...
	rfmt	= "%" + strofreal(rwidth) + "s"
	cfmt	= "%" + strofreal(cwidth-1) + "s"
	...
	for (i=r0; i<=r; i++) {
		...
		if (rsplit) {
			if (rstripe[i,1] != oldeq) {
				lastch = substr(rstripe[i,1],-1,1)
				hasop = strpos(":", lastch)
				if (hasop) {
					oldeq = substr(	rstripe[i,1],
							1,

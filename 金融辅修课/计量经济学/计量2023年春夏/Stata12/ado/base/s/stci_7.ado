*! version 7.0.14  29may2007
program define stci_7, rclass byable(recall) sort
	version 7, missing
	st_is 2 analysis
	...
	if "`ccorr'"~="" & "`rmean'"==""  {
		di as err "option ccorr allowed only with option rmean"
		exit 198	
	}
	if "`graph'"~="" & "`emean'"==""  {
		di as err "option graph allowed only with option emean"
		exit 198	
	}
	if "`graph'"~="" & "`by'"~="" {
		di as err /*
	...
	if "`tmax'"~="" & "`graph'"==""  {
		di as err "option tmax() allowed only with option graph"
		exit 198	
	}
	if "`saving'"~="" & "`graph'"==""  {
		di as err "option saving() allowed only with option graph"
		exit 198	
	}
	st_show `show'
	if "`median'"~="" | "`p'"=="50" {
	...
	if `median'<3 {
		...
		if `"`by'"' != "" {
			...
			local i 1
			while `i' <= `ng' {
				qui sts gen `N'=n if `touse' & `grp'==`i'
				qui sts gen `d'=d if `touse' & `grp'==`i'
				qui gen byte `subuse'=`touse' & `grp'==`i'
				qui compress `N' `d'
		...
		tempvar N d
		qui sts gen `N'=n if `touse'
		qui sts gen `d'=d if `touse'
		qui compress `N' `d'
...
program define MYStats, rclass /* touse atrisk by p level */
	...
	quietly {
		...
		tempname tb1 tb2
		sts gen `greenV'=se(s)
		qui sts gen `lb'=lb(s) if `touse', level(`level')
		qui sts gen `ub'=ub(s) if `touse', level(`level')
			...
			if `median'== 4 {
				...
				if `flag2'~=0 {
					...
					*/ _t[_n-1]+ (`tmax'-`mt')/20 if _n>`nn'
					...
					gr7 `s' `ns' _t, c(Js) s(..) sort /*
					 */ ylab(0,.2,.4,.6,.8,1) /*
					*/ t1("Exponentially extended" /*
					*/ "survivor function") /*
					*/ l2("Survival probability") `options'
					restore
				}
				if `lamb'>=. {
					local area2 0

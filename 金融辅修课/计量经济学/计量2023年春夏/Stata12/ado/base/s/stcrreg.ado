*! version 1.4.2  09apr2011
program stcrreg, eclass byable(onecall) ///
		prop(st hr nohr shr noshr sw mi fvaddcons)
	if replay() {
		version 11
...
program Display
	...
	if "`table'" == "" {
		...
		if "`e(mainoff)'" != "" {
			di as txt "{p 0 4 2}Note: model includes the linear "
			di as txt "offset " as res "`e(mainoff)'" 
			di as txt " in equation " as res "main{p_end}"
		}
	}
end
...
program DiHeader, rclass
	...
	di as txt _col(51) "No. of obs" 		_col(68) "=" /// 
		_col(70) as res %9.0g e(N)
	di as txt _col(51) "No. of subjects" 	_col(68) "=" ///
		_col(70) as res %9.0g e(N_sub)
	local fnum 0
	...
	di as txt _col(51) "No. failed"         _col(68) "=" ///
		_col(70) as res %9.0g e(N_fail)
	local cnum 0
	...
	di as txt _col(51) "No. competing" _col(68) "=" /// 
		_col(70) as res %9.0g e(N_compete)
	di as txt _col(51) "No. censored"         _col(68) "=" ///
		_col(70) as res %9.0g e(N_censor)
	di

	if inlist("`e(vce)'", "bootstrap", "jackknife") {
		di _col(51) as txt "Replications" _col(68) "=" /// 
			_col(70) as res %9.0g e(N_reps)
	}
	if inlist("`e(vce)'", "jackknife") {
		if missing(`e(F)') {
			local h help j_robustsingular
			di _col(51) as smcl "{`h':F(`e(df_m)', `e(df_r)')}" _c
		}
		else {
			di _col(51) as txt "F(" /// 
		...
		di _col(68) as txt "=" _col(70) as res %9.2f e(F)
		di as txt "`crtype'" " = " as res %10.0g e(ll) ///
			_col(51) as txt "Prob > F" _col(68) "=" _col(73) ///
			as res %6.4f e(p)
	}	
	else {
		if missing(`e(chi2)') {
			local h help j_robustsingular
			di _col(51) as smcl "{`h':Wald chi2(`e(df_m)')}" _c
		}
		else {
			di _col(51) as txt "`e(chi2type)' chi2(" /// 
				as res e(df_m) as txt ")" _c
		}
 		di _col(68) as txt "=" _col(70) as res %9.2f e(chi2)
		di as txt "`crtype'" " = " as res %10.0g e(ll) ///
			_col(51) as txt "Prob > chi2" _col(68) "=" _col(73) ///
			as res %6.4f e(p)
	}
...
program ParseCompete, sclass
	...
	if `"`event'"' != "" {
		...
		tokenize `event'
		while "`1'" != "" {
			qui replace `cvar' = 1 if `bd' == `1' & `touse'
			mac shift
		}
		...
		qui gen byte `cvar' = (`bd'!=0 & `bd'<.) & `touse'
		local crevent `abd' 
		local crnz crnz
		local crmult crmult
	}

...
{viewerjumpto "Saved results" "streg##saved_results"}{...}
{title:Title}

{p2colset 5 19 21 2}{...}
{p2col :{manlink ST streg} {hline 2}}Parametric survival models{p_end}
{p2colreset}{...}
...
{synopt :{cmdab:d:istribution(}{cmdab:logl:ogistic)}}loglogistic survival distribution{p_end}
...
{opt mi estimate}, {opt nestreg}, {opt statsby},
...
{cmd:stset}; see {manhelp stset ST}. However, weights may not be specified if
you are using the {cmd:bootstrap} prefix with the {cmd:streg} command.{p_end}
{p 4 6 2}
...
{pmore}
The currently supported distributions are listed in the
{help streg##options:options table}.
...
explanatory variables, type {cmd:streg,} {cmd:distribution({it:distname})}....
...
{phang}
{opt tr} specifies that exponentiated coefficients, which are
interpreted as time ratios, be displayed.  {opt tr} is appropriate only for the
loglogistic, lognormal, and gamma models, or for the exponential and Weibull
models when fit in the accelerated failure-time metric.
...
{pstd}
The following option is available with {opt streg} but is not shown in the
dialog box:
...
metric{p_end}
{phang2}{cmd:. streg load bearings, d(weibull) time}
...
{phang2}{cmd:. streg x1 x2, d(weibull) vce(robust)}

{pstd}Same as above, but fit exponential model rather than Weibull{p_end}
{phang2}{cmd:. streg x1 x2, d(exp) vce(robust)}
...
{phang2}{cmd:. streg drug age, d(gamma)}

{pstd}Test for appropriateness of Weibull model{p_end}
{phang2}{cmd:. test [kappa]_cons = 1}
...
{phang2}{cmd:. streg protect age, d(weibull) ancillary(male)}
...
{phang2}{cmd:. streg age, d(weibull) strata(drug)}

{pstd}Produce a "less-stratified" model than above{p_end}
{phang2}{cmd:. streg age, d(weibull) ancillary(i.drug)}
...
{phang2}{cmd:. streg age smoking, d(weibull) frailty(gamma)}

{pstd}Fit Weibull survival model with inverse-Gaussian-distributed frailty
{p_end}
{phang2}{cmd:. streg age smoking, d(weibull) frailty(invgauss)}
...
{phang2}{cmd:. streg age female, d(lnormal) frailty(invgauss) shared(patient)}
...
data{p_end}
{phang2}{cmd:. svy: streg former_smoker smoker male urban1 rural, d(exp)}
{p_end}
    {hline}
...
{pstd}
{cmd:streg} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(sigma)}}ancillary parameter ({cmd:gamma, lnormal}){p_end}
{synopt:{cmd:e(kappa)}}ancillary parameter ({cmd:gamma}){p_end}

...
{p2colset 5 23 25 2}{...}
{p2col :{manlink ST sts graph} {hline 2}}Graph the survivor and cumulative hazard functions{p_end}
{p2colreset}{...}
...
to plot survivor function adjusted to age of patients and the ages in your
...
requests that each group be placed on its own rather than one on top of the
other.  Sometimes curves have to be placed on separate graphs -- such as when
you specify {opt ci} -- because otherwise it would be too confusing.
...
and all remaining rows colored navy.
        ...
        {dlgtab:Main}
{phang3}
{it:axis_label_options} control table by using axis labeling
...
{p 16 20 2}
{opth topgap(relativesize)} specifies how much space is to be placed above 
each row.

{p 16 20 2}
{opth bottomgap(relativesize)} specifies how much space is to be placed beneath 
each row.
...
{p 20 24 2}
{opth bottomgap(relativesize)} specifies how much space is to be placed
beneath each row.
...
displayed are the pure number censored.  The underlying logic is

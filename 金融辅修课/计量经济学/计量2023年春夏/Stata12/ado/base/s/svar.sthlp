...
{viewerjumpto "Saved results" "svar##saved_results"}{...}
{viewerjumpto "Reference" "svar##reference"}{...}
{title:Title}

{p2colset 5 22 24 2}{...}
{p2col:{manlink TS var svar} {hline 2}}Structural vector autoregressive
models{p_end}
{p2colreset}{...}
...
{synopt :{it:{help svar##display_options:display_options}}}control column
         formats{p_end}
...
{p 4 6 2}{opt by}, {opt rolling}, {opt statsby}, and {cmd:xi} are allowed; see
{help prefix}.{p_end}
...
{pmore}
   ...
   {bf:C} matrix (see 
   {mansection TS varsvarRemarksLong-runSVARmodels:{it:Long-run SVAR models}}
   in {bf:[TS] var svar}
...
{it:{mansection TS varRemarksFittingmodelswithsomelagsexcluded:Fitting models with some lags excluded}}
in {bf:[TS] var}.
...
{pstd}
{cmd:svar} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(df_m_var)}}model degrees of freedom{p_end}
...
{synopt:{cmd:e(nocons_var)}}noconstant, if {cmd:noconstant} specified{p_end}
...
{synopt:{cmd:e(G_var)}}Gamma matrix saved by {cmd:var}; see
	{it:Methods and formulas} in {hi:[TS] var svar}{p_end}

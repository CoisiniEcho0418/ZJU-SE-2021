...
program SvySet
	...
	local ignore 0
	forval i = 1/`stage_n' {
		ParseStage "`mi'" `i' `stage_n' `ignore' `stage_`i''
		_svyset get fpc `i'
		if "`r(fpc`i')'" == "" & `stage_n' > `i' {
			di as txt "{p 0 7 2}" ///
"Note: stage `i' is sampled with replacement;" ///
" all further stages will be ignored{p_end}"
			local stage_n `i'
			local ignore 1
		}
	}
...
program ParseGlobals, sclass
	...
	if `"`singleunit'"' != "" {
		...
		if c(rc) {
			di as err "option singleunit() invalid"
			exit 198
		}
	}
	if `stage_n' == 0 & "`noclear'" == "" {
		...
		if "`mypsu'`psu'`strata'`fpc'" != "" {
			ParseStage "`mi'" 1 1 0 `mypsu', `psu' `strata' `fpc'
		}
	}
...
"observations can only be sampled in the final stage"
	...
	if !inlist("`su'", "", "_n") {
		...
		if c(rc) {
			...
			di as err `"`su' invalid `psu' variable name"'
			exit c(rc)
		}
	}
	if `ignore'	exit
...
"and is different from the value supplied to stratum(){p_end}"
				exit 459
			}
...
"and is different from the value supplied to fpc(){p_end}"
				exit 459
			}
...
"and is different from the value supplied to multiplier(){p_end}"
				exit 459
			}
...
program sdrInvalid
	args rc
	di as err "option sdrweight() incorrectly specified"
	if "`rc'" == "" {
		exit 198
	}
	else	exit `rc'

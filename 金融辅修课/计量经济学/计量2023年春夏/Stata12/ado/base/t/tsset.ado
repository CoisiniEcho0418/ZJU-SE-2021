*! version 7.3.0  09jun2011
program tsset, rclass
	version 10

	capture syntax [, MI noQUERY]
	if (_rc==0) {
		if ("`mi'"=="") {
	...
	local curftyp = substr("`curfmt'", 2, 1)
	if "`curftyp'" != "t" & "`curftyp'" != "d" {
		local curftyp g
	}
	else if "`curftyp'" == "t" {
		local curftyp = substr("`curfmt'", 3, 1)
	}
	local newftyp = substr("`format'", 2, 1)
	if "`newftyp'" != "t" & "`newftyp'" != "d" {
		local newftyp g
	}
	else if "`newftyp'" == "t" {
		local newftyp = substr("`format'", 3, 1)
	}
...
di in red "time values with period less than delta() found"
	...
	if "`query'" == "" {
		Query 		/* sets r(tmax), r(tmin), r(tdelta), etc. */
		ret add
	}
	else {
	...
	string scalar tvarname)		// name of time variable
{
	...
	if (_st_varindex(tvarname)!=.) {
		...
		if (substr(fmt,2,1)=="t") units = substr(fmt, 3, 1) 
		else if (substr(fmt,2,1)=="d") units = "d" 
	}

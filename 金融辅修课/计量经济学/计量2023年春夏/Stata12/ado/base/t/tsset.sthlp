...
{viewerjumpto "Saved results" "tsset##saved_results"}{...}
{title:Title}

{p2colset 5 19 21 2}{...}
{p2col :{manlink TS tsset} {hline 2}}Declare data to be time-series data{p_end}
{p2colreset}{...}
...
{syntab:Delta}
{synopt :{help tsset##deltaoption:{it:deltaoption}}}specify period of {it:timevar}
{p_end}
...
{pstd}
{cmd:tsset} declares the data in memory to be a time series. 
{cmd:tsset}ting the data is what makes Stata's
{help tsvarlist:time-series operators} such as
{cmd:L.} and {cmd:F.} (lag and lead) work.  Also, before using the other
{cmd:ts} commands, you must {cmd:tsset} the data first.  If you {cmd:save} the
data after {cmd:tsset}, the data will be remembered to be time series and you
will not have to {cmd:tsset} again.

{pstd}
There are two syntaxes for setting the data:

		{cmd:tsset} {it:timevar}

		{cmd:tsset} {it:panelvar} {it:timevar}

{pstd}
In the first syntax -- {cmd:tsset} {it:timevar} -- the data are set to be
a straight time series.

{pstd}
In the second syntax -- {cmd:tsset} {it:panelvar} {it:timevar} -- the data are
set to be a collection of time series, one for each value of {it:panelvar},
also known as panel data, cross-sectional time-series data, and {cmd:xt} data.
Such datasets can be analyzed by {cmd:xt} commands as well as {cmd:ts}
commands.  If you {cmd:tsset} {it:panelvar} {it:timevar}, you do not need to
{cmd:xtset} {it:panelvar} {it:timevar} to use the {cmd:xt} commands.

{pstd}
{cmd:tsset} without arguments -- {cmd:tsset} -- displays how the data are
currently {cmd:tsset} and sorts the data on {it:timevar} or {it:panelvar}
{it:timevar} if they are sorted differently from that.
...
{pmore}
    ...
    numerical format other than {cmd:%t} (or if it has a {cmd:%tg} format).
...
{phang}
{cmd:delta()} specifies the period of {it:timevar} and is commonly used
    when {it:timevar} is {cmd:%tc}.  {cmd:delta()} is only sometimes used with
    the other {cmd:%t} formats or with generic time variables.
...
{pmore}
    The are two aspects of {it:timevar}:  its units and its periodicity.  The
    {it:unitoptions} set the units.  {cmd:delta()} sets the periodicity.
...
{pstd}
{cmd:tsset} saves the following in {cmd:r()}:
...
{synopt:{cmd:r(balanced)}}{cmd:unbalanced}, {cmd:weakly balanced}, or
	     {cmd:strongly balanced}; a set of panels are strongly balanced if
	     they all have the same time values, otherwise balanced if same
	     number of time values, otherwise unbalanced{p_end}
{p2colreset}{...}

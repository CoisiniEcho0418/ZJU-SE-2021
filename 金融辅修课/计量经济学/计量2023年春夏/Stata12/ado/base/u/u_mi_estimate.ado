...
program CheckPrefix, sclass
	...
	local prefix  = rtrim(ltrim(`"`s(before)'"'))
	local command = rtrim(ltrim(`"`s(after)'"'))
	_parse comma prefix prefixopts : prefix
	gettoken prefix prefixrest : prefix
	if ("`prefix'"=="xtmixed"|"`prefix'"=="xtmelogit" |	///
	    "`prefix'"=="xtmepoisson"|"`prefix'"=="ratio") {
...
program CheckCommand, sclass
	...
	gettoken command : command, parse("||") //for -xtmixed- with weights
	local 0 `command'

	syntax anything(id="command name" equalok) 	///
		[fw iw pw aw] [if] [in] [, VCE(string asis) * ]
	
	if ("`prefix'"!="") {
		if ("`prefix'"!="svy") {
			di as err `"command prefix {bf:`prefix'} not allowed"'
			exit 198
		}
		...
		local command	`"`s(command)'"'
		if ("`prefix2'"!="") {
			di as err `"command prefix {bf:`prefix2'} not allowed"'
			exit 198
		}
...
program _save_esample_flong, sortpreserve
	...
	foreach m of local imps {
		...
		local N = r(N)
		gen long `id' = _n if _mi_m==`m' & _mi_id==1
		summ `id', meanonly
		local istart = r(mean)

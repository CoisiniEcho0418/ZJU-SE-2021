	...
	while (`continue') {
		...
		if ("`method'"=="intreg" & !`noivar') {
			if (`:word count `ivarscurr''>1) {
				...
				cap noi u_mi_impute_cmd_intreg_parse 	///
...
di as txt "note: {bf:`weight'}s applied to the model for {bf:`ivar'}"
		...
		local currterms : list uniq currterms
		...
		local fvnotallowed : list fvnotallowed - fvivar
		...
		cap noi `showchk' u_mi_impute_cmd_`method'_parse	///
		...
		if ("`noimputed'`noimputed_gl'"=="") { 
			...
			local rhsmon`j' : list rhsmon`j' - ivmonomit
			local dirhsmon`j' : list dirhsmon`j' - ivmonomit
			if (!`domonotone') {
				local rhs`j' : list rhs`j' - ivomit
				local dirhs`j' : list dirhs`j' - ivomit
			}
		}
		...
		if ("`report'"!="" & `domonotone') {
			...
			if ("`noimputed'`noimputed_gl'"!="") {
				local ltmp : list rhsterms & ivmonomit
			...
			local omittedterms `s(ivarexplists)'
			if ("`noimputed'`noimputed_gl'"!="") {
				local ltmp : list rhsterms & ivomit
				local omittedterms `ltmp' `omittedterms'
			}
			local omittedterms : list uniq omittedterms
			if ("`omittedterms'"!="") {

...
program u_mi_predictions
	...
	if (`M_max'>`M') {
		...
		di as err "results correspond to a different {bf:mi} data."
		exit 459
	}
	...
	if ("`2'"!="") {
		...
		if !inlist("`vtype'","byte","int","long","float","double") {
			di as err `"`vtype' is not a valid numerical data type"'
			exit 198
		}
	}
	else {
	...
	if ("`esample'"!="") {
		...
		if _rc {
			di as err "{bf:esample()} must contain a binary " ///
				  "variable"
			exit 459
		}
		...
		if (`isregistered') {
			di as err "{p 0 2 2}{bf:esample()}: variable"
	...
	if ("`storecompleted'"!="" & ("`style'"=="wide" | "`style'"=="mlong")) {
		di as err "{p}{bf:storecompleted} is allowed only in {bf:flong}"
...
program _mi_predictnl_parse, sclass
	...
	foreach newvaropt in `newvaropts' {
		if ("``newvaropt''"!="") {
			...
			if (`:word count ``newvaropt''' != 1) {
				di as err "{p}{bf:`newvaropt'()} requires one"
	...
	if (`"`ci'"'!="") {
		if (`:word count `ci'' != 2) {
			di as err "{p}{bf:ci()} requires two new variable " ///

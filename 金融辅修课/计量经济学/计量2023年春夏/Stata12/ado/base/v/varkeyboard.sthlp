...
{phang}Set the keyboard navigation behavior for the Variables and Review windows
(Mac only)
...
Review window.  These commands apply only to Mac computers.

{pstd}
{cmd:on} indicates that you can use the keyboard to navigate and enter items
from the Variables or Review window into the Command window.  {cmd:off}
indicates that all keyboard input be directed at the Command window; items
can only be entered from the Variables or Review window by using the mouse.

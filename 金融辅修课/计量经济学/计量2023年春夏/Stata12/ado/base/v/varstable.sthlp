...
{viewerjumpto "Saved results" "varstable##saved_results"}{...}
{title:Title}

{p2colset 5 23 25 2}{...}
{p2col:{manlink TS varstable} {hline 2}}Check the stability condition of VAR
...
{opt estimates(estname)} requests that {opt varstable} use the
   previously obtained set of {cmd:var} estimates saved as {it:estname}.  By
   default, {opt varstable} uses the active estimation results.
   See {manhelp estimates R} for information on saving and restoring estimation
   results.
...
{cmd:pgrid(}[{it:{help numlist}}][{cmd:,} {it:line_options}]{cmd:)}
   ...
   have the {opt grid} line style.  The {it:numlist} specifies the radii for
   the polar grid circles.  The {it:line_options} determine the
   appearance of the polar grid circles; see {manhelpi line_options G-3}.
   Because the {opt pgrid()} option can be repeated, circles with different
   radii can have distinct appearances.
...
{pstd}
{cmd:varstable} saves the following in {cmd:r()}:

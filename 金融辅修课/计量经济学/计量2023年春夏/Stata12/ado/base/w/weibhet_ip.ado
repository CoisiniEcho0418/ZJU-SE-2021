...
program define weibhet_ip, sort
	version 7, missing
...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		if "`e(frm2)'"=="time" {
			qui replace `xb' = -`xb'*`auxp' if `touse'
		}
		gen `vtyp' `varn'=(((1-`theta'*ln(0.5))^2 - 1)/(2*`theta') /*
			*/ *exp(-`xb'))^(1/`auxp') if `touse'
...
*/ "(option log median time assumed; predicted median log time)"
	...
	if "`type'"=="hazard" {
		...
		qui gen double `kk' = exp(`xb')*`t'^(`auxp'-1) if `touse'
		qui gen double `ff'= `kk'*`auxp'/ /*
	...
	if "`type'" =="surv" {
		...
		qui gen double `ff'= 1+2*`theta'*exp(`xb')*`t'^`auxp' /*
			*/ if `touse'
		qui gen double `ff0'= 1+2*`theta'*exp(`xb')*`t0'^`auxp' /*
			*/ if `t0'>0 & `touse'
	...
	if "`type'"=="mgale" { 
		...
		qui gen double `ff'= /*
		*/ (`d'!=0)+(1/`theta')* /*
		*/ (1-sqrt(1+2*`theta'*exp(`xb')*`t'^`auxp')) if `touse'
		qui replace /*
		*/ `ff'=`ff'-(1/`theta')* /*

*! version 7.1.5  19apr2007
program define wilc_st, rclass 
	version 6, missing
	...
	if `"`strata'"' != `""' {
		if `"`detail'"' != `""' {
			...
			di 
			di in gr `"-> Total"'
			wilc_st `varlist' [`weight'`exp'] if `touse' /*
	...
	quietly { 
		...
		while `i' <= `ng' {
			...
			while `j'<=`ng' {
				...
				gen double `wi' = sum((((`n')^2*`ni'*`d'*(`n'-`d')) / /*
				*/ (`n'*(`n'-1)) )*((`i'==`j') - `nj'/`n'))
				mat `V'[`i',`j'] = `wi'[_N]
				mat `V'[`j',`i'] = `wi'[_N]
				drop `wi'
...
*/ `"{title:Wilcoxon (Breslow) test for equality of survivor functions}"'
		}
		else	di in smcl _n(2) in gr /*
*/ `"{title:Stratified Wilcoxon (Breslow) test for equality of survivor functions}"'
	}
end
exit

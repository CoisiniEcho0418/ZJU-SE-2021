...
{p2colset 5 22 24 2}{...}
{p2col :{hi:window manage} {hline 2}}Manage window characteristics{p_end}
{p2colreset}{...}
...
{phang}Refresh the Variables window

{p 8 18 2}
{opt win:dow} {opt man:age} {opt update} {opt variable}
...
{opt help}, {opt results}, {opt review}, {opt variables}, or {opt viewer}.
...
Remarks are presented under the following headings:
       ...
       {hi:{help window manage##minimizing:Minimizing or restoring the main window (Mac and Windows only)}}
...
{marker minimizing}{...}
{title:Minimizing or restoring the main window (Mac and Windows only)}

{pstd}
{cmd:window} {cmd:manage} {cmd:minimize} minimizes (hides) the Stata window.
With Stata for Windows, this has the same effect as clicking on the minimize
button on Stata's title bar.  With Stata for Mac, this has the same
effect as selecting {hi:Hide Stata} from the {hi:Stata} menu.
...
{pstd}
minimizes the overall Stata window if you are using Stata for Windows and
hides Stata's windows if you are using Stata for Mac.

{pstd}
{cmd:window} {cmd:manage} {cmd:restore} restores the Stata window if
necessary.  This command is allowed only with Stata for Mac and Stata for
Windows.
...
{bf:Edit > Preferences > Manage Preferences > Load Preferences} and loading a
named preference set.  
{cmd:window manage prefs save} "{it:prefname}" is equivalent to selecting
{bf:Edit > Preferences > Manage Preferences > Save Preferences} and naming
a new preference set.  
{cmd:window manage prefs default} is equivalent to selecting
{bf:Edit > Preferences > Manage Preferences > Load Preferences > Factory Settings}.
...
{bf:Edit > Preferences > Restore File Associations}.
...
{pstd}
In Stata for Unix (GUI) and Stata for Windows, {cmd:window} {cmd:manage}
...
understands:
          ...
          {cmd:review}{col 30}Review window
...
{pstd}
Under Stata for Unix, specifying the Command, Results, Review, or Variables
windows will bring the main Stata window forward because these windows are
all contained within one window.
{p_end}
...
{pstd}
{cmd:window} {cmd:manage} {cmd:rename} {it:oldgraphname} {it:newgraphname}
renames the Graph window named {it:oldgraphname}, if it exists, to 
{it:newgraphname}.  Note that this is not intended for an end user to use;
it is a utility to be used by {cmd:graph rename}.  End users should use
{cmd:graph rename} to rename graphs.

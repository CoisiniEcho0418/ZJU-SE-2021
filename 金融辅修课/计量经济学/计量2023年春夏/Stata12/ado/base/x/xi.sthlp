...
{viewerjumpto "Saved results" "xi##saved_results"}{...}
{title:Title}

{p2colset 5 15 17 2}{...}
{p2col :{manlink R xi} {hline 2}}Interaction expansion{p_end}
{p2colreset}{...}
...
{p 4 6 2}- {cmd:xi} creates new variables in your data; most are {hi:byte}s,
but interactions with continuous variables will have the storage type of the
underlying continuous variable; see {manhelp data_types D:data types}.

{p 4 6 2}- when using {cmd:xi} with an estimation command, you may get the
message "{err:matsize too small}".  If so, see {manhelp matsize R}.
...
{pstd}
{cmd:xi} saves the following characteristics:

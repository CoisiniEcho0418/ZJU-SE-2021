...
{viewerjumpto "Saved results" "xtabond##saved_results"}{...}
{viewerjumpto "References" "xtabond##references"}{...}
{title:Title}

{p2colset 5 21 23 2}{...}
{p2col :{manlink XT xtabond} {hline 2}}Arellano-Bond linear dynamic panel-data estimation{p_end}
{p2colreset}{...}
...
inconsistent.  {help xtabond##AB1991:Arellano and Bond (1991)} derived a
consistent generalized
method of moments (GMM) estimator for the parameters of this model;
{cmd:xtabond} implements this estimator.

{pstd}
This estimator is designed for datasets with many panels and few
periods, and it requires that there be no autocorrelation in the
idiosyncratic errors.  For a related estimator that uses additional moment
conditions, but still requires no autocorrelation in the idiosyncratic
errors, see {manhelp xtdpdsys XT}.  For estimators that allow for some
autocorrelation in the idiosyncratic errors, at the cost of a more
complicated syntax, see {manhelp xtdpd XT}.
...
For predetermined variables, the default is to use all T_i-p-1 lags of these
endogenous variables.  For endogenous variables,
the default is to use all T_i-p-2 lags of these variables.
...
{mansection XT xtabondRemarks:{it:Remarks}} in {bf:[XT] xtabond}.
...
{pstd}
{cmd:xtabond} saves the following in {cmd:e()}:

...
{viewerjumpto "Syntax" "xtcloglog##syntax"}{...}
{viewerjumpto "Description" "xtcloglog##description"}{...}
{viewerjumpto "Options for RE model" "xtcloglog##options_re"}{...}
{viewerjumpto "Options for PA model" "xtcloglog##options_pa"}{...}
{viewerjumpto "Technical note" "xtcloglog##technote"}{...}
{viewerjumpto "Examples" "xtcloglog##examples"}{...}
{viewerjumpto "Saved results" "xtcloglog##saved_results"}{...}
{title:Title}

{p2colset 5 23 25 2}{...}
{p2col :{manlink XT xtcloglog} {hline 2}}Random-effects and population-averaged cloglog models{p_end}
{p2colreset}{...}
...
{synopt :{it:{help xtcloglog##re_display_options:display_options}}}control
...
{syntab:Correlation}
{synopt :{cmdab:c:orr(}{it:{help xtcloglog##correlation:correlation}}{cmd:)}}within-group correlation structure{p_end}
...
{synopt :{opt s:cale(parm)}}override the default scale parameter; {it:parm} may be {cmd:x2}, {cmd:dev}, {cmd:phi}, or {it:#}{p_end}

{syntab:Reporting}
{synopt :{opt level(#)}}set confidence level; default is {cmd:level(95)}{p_end}
{synopt :{opt ef:orm}}report exponentiated coefficients{p_end}
{synopt :{it:{help xtcloglog##pa_display_options:display_options}}}control
INCLUDE help shortdes-displayoptall
...
{p 4 6 2}
A panel variable must be specified.  For {cmd:xtcloglog, pa}, correlation
...
{p 4 6 2}
See {manhelp xtcloglog_postestimation XT:xtcloglog postestimation} for
...
{bf:Statistics > Longitudinal/panel data > Binary outcomes >}
    {bf:Complementary log-log regression (RE, PA)}
...
cloglog models may be fit with {cmd:cloglog} with indicator variables for
the panels.  However, unconditional fixed-effects estimates are biased.
...
{manhelp xtgee XT} for information on fitting other population-averaged models.
...
{pstd}
The following option is available with {opt xtcloglog} but is not shown in the
dialog box:
...
{pstd}
Because the {cmd:xtcloglog} likelihood function is calculated by Gauss-Hermite
...
{pstd}Population-averaged model with robust variance{p_end}
...
{pstd}
{cmd:xtcloglog, re} saves the following in {cmd:e()}:
...
{pstd}
{cmd:xtcloglog, pa} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(link)}}{cmd:cloglog}; link function{p_end}

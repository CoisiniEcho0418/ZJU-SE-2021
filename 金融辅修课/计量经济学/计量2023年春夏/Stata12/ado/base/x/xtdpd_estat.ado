...
program xtdpd_estat, rclass
	   ...
	   "`e(engine)'" != "xtdpd"   {
        ...
        if "`sub'" == substr("abond",1,max(2,`lsub')) {		// ABond
                ABond `rest'
        }
        else if "`sub'" == substr("sargan",1,max(3,`lsub')) { 	// SARgan
                SARgan `rest'
        }
...
program define ABond, rclass
	...
	if `artests' > 0 {
		...
		matrix `armv' = `order', `armv'
		matrix colnames `armv' = order arstat

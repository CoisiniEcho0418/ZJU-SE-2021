*! version 1.7.1  25may2011
program define xtfrontier, eclass byable(onecall) prop(xt)
	if _by() {
...
program define xtsfEST, eclass byable(recall) sortpreserve
	...
	if "`e(model)'" == "tvd" {
        ...
        eret hidden local diparm`++i' ilgtgamma
        eret hidden local diparm`++i' __sep__
        eret hidden local diparm`++i' lnsigma2, exp label(sigma2)
        eret hidden local diparm`++i' ilgtgamma, ilogit label(gamma)
        eret hidden local diparm`++i' lnsigma2 ilgtgamma, /*

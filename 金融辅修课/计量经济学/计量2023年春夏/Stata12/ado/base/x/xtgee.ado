*! version 4.7.3  20jun2011
program define xtgee, eclass byable(onecall) prop(xt mi)
	if _caller() >= 11 {
...
program define TSEstimate, eclass byable(recall)
	...
	syntax varlist(ts fv) [if] [in] [iw fw pw] [, HASCONS /*
		...
		*/	EForm LEVel(cilevel) Link(passthru) *]
...
*/ _newline "options t() and i() -- use xtset to change them"
			exit 198
		}
		local i "i(`myi')"
		local t "t(`myt')"			
	...
	`vv' _rmcoll `ind' if `touse' [`weight'`exp'] , `inclcon' `fvexp'
	local ind `r(varlist)'
	...
	Estimate `dep' `ind' if `touse' [`weight'`exp'], `hascons' /*
		*/	`constant' `options' `link' depname(`depname') `i' `t'
...
program define Estimate, eclass byable(recall) sort
	...
	syntax varlist(fv) [if] [in] [iw fw pw] [, /*
		...
		*/ noLOg noDISplay SCore(string) FORCE /*
	...
	if "`rgf'" != "" {
		if "`weight'" != "pweight" & "`robust'" =="" {
			noi di in red "rgf may only be specified with "/*
				*/ "vce(robust)"
			error 198
		}
		if "$S_2" != "gauss" {
...
program define KeepMode /* ivar tvar [force] */
	...
	qui {
		...
		tempvar freq bad
		sort `touse' `bb'
		by `touse' `bb': gen `freq'=_N if _n==_N & `touse'

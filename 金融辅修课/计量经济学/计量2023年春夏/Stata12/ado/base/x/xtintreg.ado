*! version 2.10.10  15feb2011
program define xtintreg, eclass byable(onecall) prop(xt)
	version 6.0, missing
...
program define Estimate, eclass byable(recall) sort
	...
	#delimit ;
	syntax [varlist(ts fv)] [if] [in] [iweight/] 
			, call(real) [I(varname) RE FE PA noSKIP noREFINE intreg
		...
		*/
		...
		if "`from'" == "" {
			...
			if ("`constr'" != "" & "`coll'"!="" & `nn'>`cb') {
				...
				local cnms "`cnms' sigma_u:_cons sigma_e:_cons"
				`vv' ///
		...
		if "`skip'" != "" {
			`lll' di in gr _n "Obtaining starting values for " /*
				*/ "constant-only model:"
			`vv' ///
			`lll' xtreg `ys' if `touse', i(`ivar') mle nodisp sortst 
			mat `beta0' = e(b)
		/// Calculate logF (by panel) used to prevent underflow
			tempname se_p se
			mat `se_p' = `beta0'[1, "sigma_e:_cons"]
			scalar `se' = `se_p'[1,1]
			tempvar xb
	...
	if "`skip'" != "" {
		...
		`lll' ml model d2 xtintreg_d2 			           ///
			...
			maximize `nolog' `mlopt' missing search(off)	   ///

...
{viewerjumpto "Saved results" "xtlogit##saved_results"}{...}
{title:Title}

{p2colset 5 21 23 2}{...}
{p2col :{manlink XT xtlogit} {hline 2}}Fixed-effects, random-effects, and population-averaged logit models{p_end}
{p2colreset}{...}
...
{synopt:{opt nodis:play}}do not display the header and coefficients{p_end}
...
{syntab :Correlation}
{synopt :{cmdab:c:orr(}{it:{help xtlogit##correlation:correlation}}{cmd:)}}within-group correlation structure{p_end}
...
population-averaged model, and {opt iweight}s are allowed with the
fixed-effects and random-effects models; see {help weight}.  Weights must be
constant within panel.{p_end}
{p 4 6 2}
{opt nodisplay} and {opt coeflegend} do not appear in the dialog box.{p_end}
{p 4 6 2}
...
{cmd:xtlogit, pa} assumes {cmd:corr(exchangeable)}.  See {manhelp xtgee XT} for
...
{pstd}
{cmd:xtlogit, re} saves the following in {cmd:e()}:
...
{pstd}
{cmd:xtlogit, fe} saves the following in {cmd:e()}:
...
{pstd}
{cmd:xtlogit, pa} saves the following in {cmd:e()}:

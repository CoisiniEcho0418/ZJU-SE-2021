...
program Estimate, sortpreserve eclass byable(recall)
	...
	local globopts `"`globopts' grouponly getblups(string) REISE"'
...
// Out of the variance estimates, count how many sigmas and rhos 
	...
	forval i = `=e(k_f)+1'/`e(k)' {
		...
		if substr("`eq'",1,3) == "lns" | substr("`eq'",1,5) == "r_lns" {
			local ++k_rs
		}
	}
...
// Robust variance estimators
	...
	_xtmixed_display, level(`level') `lrtest' ///
		...
		`grouponly' `fetable' `retable' `diopts'
end
...
program GroupStats, rclass
	...
	qui {
		bysort `touse' `namelist': /// 
			gen long `T' = cond(_n==_N,sum(`obsvar'),.) if `touse'
		sum `T' `wopt' if `touse'
	}
...
program ParseVarStruct, sclass
	...
	else if `"`struct'"' == substr("toeplitz", 1, max(2, `l')) {
	...
	else if `"`struct'"' == substr("banded", 1, max(2, `l')) {
		local resid banded
		if "`rest'" == "" {
			local ba_p -1
			local specified 0
	...
	else if `"`struct'"' == substr("exchangeable", 1, max(2, `l')) {
		local resid exchangeable
		local structlab Exchangeable
	}
	else if `"`struct'"' == substr("unstructured", 1, max(2, `l')) {
		local resid unstructured
		local structlab Unstructured
	}
	else if `"`struct'"' == substr("exponential", 1, max(3, `l')) {
		local resid exponential
		local structlab Exponential
	}
	else if `"`struct'"' == substr("independent", 1, max(3, `l')) {
...
program ProcessTimeVar, sort
	...
	if "`struct'" == "ar"  | "`struct'" == "ma" | 		/// 
	   "`struct'" == "toeplitz" {
		...
		drop `diff'
		if "`struct'" != "toeplitz" {
			qui by `gr': gen `diff' = _N > `order' if `touse'
...
di as err "{cmd:xtmixed} model{p_end}"
		exit 301
	}
...
program RescaleWeights, sortpreserve
	...
	else if `"`method'"' == substr("gk", 1, max(2, `l')) {
		tempvar wt01

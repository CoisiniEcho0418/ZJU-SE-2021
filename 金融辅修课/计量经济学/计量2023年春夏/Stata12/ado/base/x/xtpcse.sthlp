...
{viewerjumpto "Saved results" "xtpcse##saved_results"}{...}
{viewerjumpto "Reference" "xtpcse##reference"}{...}
{title:Title}

{p2colset 5 20 22 2}{...}
{p2col :{manlink XT xtpcse} {hline 2}}Linear regression with panel-corrected standard errors{p_end}
{p2colreset}{...}
...
{p 8 15 2}
{cmd:xtpcse} {depvar} [{indepvars}] {ifin} {weight} [{cmd:,} {it:options}]
...
by OLS or Prais-Winsten regression.  When computing the standard errors and the
variance-covariance estimates, {cmd:xtpcse} assumes that the disturbances are,
by default, heteroskedastic and contemporaneously correlated across panels.
...
{pmore}
All above methods are consistent and asymptotically equivalent; this is
...
variance) and that the disturbances are contemporaneously correlated across
the panels (each pair of panels has their own covariance).  This is the
standard model for PCSE.
...
{help xtpcse##G2012:Greene (2012, 280)}
...
{pstd}
{cmd:xtpcse} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(n_sigma)}}obs used to estimate elements of {cmd:Sigma}{p_end}
...
{marker G2012}{...}
{phang}
Greene, W. H. 2012. {browse "http://www.stata.com/bookstore/ea.html":{it:Econometric Analysis}. 7th ed.}

*! version 1.2.0  15jan2007
program define xtprobit_d2
	version 8.0
	...
	if "$XTP_noadap"=="" {
		...
		shat($XTP_shat) hh($XTP_hh) todo(`todo') probit 	///
		avar($XTP_qavar) wvar($XTP_qwvar) $XTP_madapt logF($XTP_logF)
	}
	else {
 		_XTLLCalc  $ML_y $ML_x1 in `jbeg'/`jend',		///
		xbeta(`theta1') w(`w') lnf(`lnfv') b(`b') g(`g') 	///
		negH(`negH') quad($XTP_quad) ivar(`ivar') $ML_xc1 	///
		$XTP_noadap todo(`todo') probit 			///

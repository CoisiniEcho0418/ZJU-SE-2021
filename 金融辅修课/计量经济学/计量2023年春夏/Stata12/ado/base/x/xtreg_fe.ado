*! version 1.8.3  21jun2011
program define xtreg_fe, eclass sort
...
*/			
			...
			keep `varlist' `ivar' `touse' `usrwgt' `cluster'
			...
			if `k' {
				...
				forvalues i = 1/`k' {
					...
					replace `junk' = `name'
					by `ivar': replace `junk' = (`name'-`name'[_n-1])
					summ `junk', meanonly
					if r(min)!=r(max) {
			...
			* we could avoid this if only we knew dfe in advance
			...
			est repost, esample(`mysamp') buildfvinfo

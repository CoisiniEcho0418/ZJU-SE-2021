...
{viewerjumpto "Saved results" "xtregar##saved_results"}{...}
{viewerjumpto "References" "xtregar##references"}{...}
{title:Title}

{p2colset 5 21 23 2}{...}
{p2col :{manlink XT xtregar} {hline 2}}Fixed- and random-effects linear models with an AR(1) disturbance{p_end}
{p2colreset}{...}
...
{p 8 16 2}
{cmd:xtregar} {depvar} [{indepvars}] {ifin} {weight} {cmd:, fe} [{it:options}]
...
{pstd}
{cmd:xtregar} fits cross-sectional time-series regression models when
the disturbance term is first-order autoregressive.  {cmd:xtregar} offers a
within estimator for fixed-effects models and a GLS estimator for random-effects
models.  Consider the model

{center:y_it = a + x_it * B + u_i + e_it}

    where

{center:e_it = rho * e_i,t-1 + z_it}

{pstd}
and where |rho| < 1 and z_it is independent and identically distributed
(i.i.d.) with mean 0 and variance sigma_z*sigma_z.  If u_i are assumed to be
fixed parameters, the model is a fixed-effects model.  If u_i are assumed
to be realizations of an i.i.d. process with mean 0 and variance
sigma_z*sigma_z, it is a random-effects model.  Whereas in the fixed-effects
model, the u_i may be correlated with the covariates x_it, in the
random-effects model the u_i are assumed to be independent of
the x_it.  On the other hand, any x_it that do not vary over t are collinear
with the u_i and will be dropped from the fixed-effects model.  In contrast,
the random-effects model can accommodate covariates that are constant over
time.

{pstd}
{cmd:xtregar} can accommodate unbalanced panels whose observations are
unequally spaced over time.  {cmd:xtregar} implements the methods derived in
{help xtregar##BW1999:Baltagi and Wu (1999)}.
...
{p 12 25 2}{cmd:dw} {space 7} rho_dw = 1 - dw/2, where dw is the Durbin-Watson d
		statistic{p_end}
...
these methods are given in {bf:[TS] prais}.  {opt prais} handles unequally
...
{phang}
{opt rhof(#)} specifies that the given number be used for rho and that rho is
not to be estimated.
...
{pstd}
{cmd:xtregar, re} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(dw)}}LBI, if requested{p_end}
...
{pstd}
{cmd:xtregar, fe} saves the following in {cmd:e()}:

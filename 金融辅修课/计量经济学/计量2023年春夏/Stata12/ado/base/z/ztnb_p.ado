*! version 1.1.0   02jun2009
program define ztnb_p
	version 8.0
	...
	if "`type'"=="cm"{
        	if (e(dispers)=="mean"){ 
			tempvar xb d1 alpha
                	scalar `alpha'=exp([lnalpha]_cons)
			qui _predict double `xb' if `touse', xb `offset'
                	qui gen double `d1'=1-(1+`alpha'*exp(`xb'))^ /*
                         */ (-1/`alpha') if `touse'
                }
                else if (e(dispers)=="constant"){
        		tempvar xb d1 delta
                	scalar `delta'=exp([lndelta]_cons)
			qui _predict double `xb' if `touse', xb `offset'
                	qui gen double `d1'=1-(1+`delta')^(-exp(`xb')/`delta')/*
                        */ if `touse'
                }

...
quietly {
	...
	matrix `b' = e(b)
	matrix `V' = e(V)
	if "`setype'" != "count" & "`e(V_srs)'" == "matrix" {
		tempname Vsrs
		if "`srssub'" == "" {
			matrix `Vsrs' = e(V_srs)
		}
...
program MakeFull, eclass // Fuller et al. variant
	...
	ereturn scalar F_`name'   = `stat'/`tr'
	ereturn scalar df1_`name' = min(`tr'^2/`tr3',`dfnom')
	ereturn scalar df2_`name' = e(df_r)
	ereturn scalar p_`name'   = fprob(e(df1_`name'),e(df2_`name'),e(F_`name'))
end
...
program CheckSyntax
	...
	if "`se'`cv'`ci'`deff'`deft'" != "" {
		...
		if `nopts' > 1 {
			...
			di as err ///
"only one of cell, count, row, or column can be specified when `opt' specified"
			exit 198
		}
	}
	if "`stdize'" != "" & "`count'`wald'" != "" {
		...
		di as err ///
"option `opt' is not allowed with direct standardization"
		exit 198
	}
...
"options deff and deft are not allowed with poststratification"
		...
		if `"`e(stdize)'"' != "" {
			if "`deff'`deft'" != "" {
				di as err ///
"options deff and deft are not allowed with direct standardization"
				exit 198
			}
			if "`count'" != "" {
				di as err ///
"option count is not allowed with direct standardization"
				exit 198
			}
		}
		...
		if "`wald'" != "" & "`e(F_Wald)'" == "" {
			di as err "{p 0 0 2}"		///
"wald not available on redisplay; it must be specified at run time{p_end}"
			exit 198
		}
...
"option srssubpop requires subpopulation estimation results"
		...
		if "`se'`cv'`ci'`deff'`deft'" != ""		///
		 & "`e(setype)'" != "`cell'`count'`row'`column'" {
			if "`se'" != "" {
				di as err "{p 0 0 2}"	///
"standard errors are only available for `e(setype)'s"	///
" to compute `cell'`count'`row'`column' standard"	///
" errors, rerun command with `cell'`count'`row'`column' and se options{p_end}"
				exit 111
			}
			if "`cv'" != "" {
				di as err "{p 0 0 2}"	///
"coefficients of variation are only available for"	///
" `e(setype)'s to compute `cell'`count'`row'`column'"	///
" coefficients of variation, rerun command with"	///
" `cell'`count'`row'`column' and cv options{p_end}"
				exit 111
			}
			if "`ci'" != "" {
				di as err "{p 0 0 2}"	///
"confidence intervals are only available for"		///
" `e(setype)'s to compute `cell'`count'`row'`column'"	///
" confidence intervals, rerun command with"		///
" `cell'`count'`row'`column' and ci options{p_end}"
				exit 111
			}
			...
			di as err "{p 0 0 2}"		///
"`deff' is only available for `e(setype)'s to compute"	///
" `cell'`count'`row'`column' `deff', rerun command"	///
" with `cell'`count'`row'`column' and `deff' options{p_end}"
			exit 111
		}
...
"options deff and deft are not allowed with direct standardization"
		...
		if "`ci'" != "" & "`vertical'" != "" & `ncell' > 2 {
			di as err "{p 0 0 2}"		///
"only 2 of se, cv, deff, deft, and obs can be specified"	///
" when ci and vertical are specified{p_end}"
			exit 198
		}
		if "`ci'" != "" {
			local ncell = `ncell' + 1
		...
		if "`vertical'" == "" & `ncell' > 4 {
			di as err ///
"only 4 of se, cv, ci, deff, deft, and obs can be specified"
			exit 198
		}
...
program CheckRowCol
	...
	if (`nrow' == 1 | `ncol' == 1) & "`rowcol'" != "" {
		if `ncol' == 1 & substr("`rowcol'",1,3) == "row" {
			di as err ///
	"option row is not allowed for tables with a single column"
			exit 198
		}
		if `nrow' == 1 & substr("`rowcol'",-6,.)  == "column" {
			di as err ///
	"option column is not allowed for tables with a single row"
			exit 198
		}
	}
end
...
program DispTab
	...
	if "`format'" == "" {
		if "`cellw'" != "" {
			local format "%`cellw'.0g"
		}
		else	local format "%6.0g"
		local format "%6.0g"
	}
...
quietly {
	...
	if "`label'" == "" {
		...
		if "`e(rowlab)'" != "" { 
			tempname lblname
			MakeLab `lblname' `rowvar' `Row' 
		}
		if "`e(collab)'" != "" {
			tempname lblname
			MakeLab `lblname' `colvar' `Col'
		}
	}

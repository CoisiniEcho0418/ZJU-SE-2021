...
{p2colset 5 22 24 2}{...}
{p2col:{manlink MV Glossary} {hline 2}}Glossary of terms{p_end}
{p2colreset}{...}
...
of the data is in one cluster.  This process creates a hierarchy of clusters.
Contrast to 
...
matrix is the KMO measure of sampling adequacy for the individual
...
{phang}
{bf:Bayes' theorem}.
Bayes' theorem states that the probability of an event, A, conditional on
another event, B, is generally different from the probability of B
conditional on A, although the two are related.   Bayes' theorem is that
...
used in multivariate analysis of variance and related hypothesis tests: Wilk's
lambda, Roy's largest root, Lawley-Hotelling trace, and Pillai's trace.
...
{phang}
{bf:centered data}.
Centered data has zero mean.  You can center data {bf:x} by taking
{bf:x} - {bf:x}[bar].
...
Also see 
{help mv_glossary##LDA:LDA} and
{help mv_glossary##QDA:QDA}.
...
{pmore}
Tandem principal 1 minimizes the criterion 

{phang3}
 c({bf:Lambda}) = < {bf:Lambda}^2, ({bf:LambdaLambda}')^2{bf:Lambda}^2>

{pmore}
Tandem principal 2 minimizes the criterion
...
Also see {help mv_glossary##CA:CA}.
...
to LDA.  See {help mv_glossary##LDA:LDA}.
...
clustering.  All the data begins as a part of one large cluster; with
each iteration, a cluster is broken into two to create two new clusters.  At the
first iteration there are two clusters, then three, and so on.  Divisive
methods are very computationally expensive.  Contrast
to 
...
In hierarchical clustering, the data is placed into clusters via
...
Also see 
{help mv_glossary##CA:CA} and
{help mv_glossary##MCA:MCA}.
...
linear combinations of the discriminating variables that provide maximal
separation between the groups.  The 
{help mv glossary##M1936:Mahalanobis (1936)} formulation of
LDA assumes that the observations come from a multivariate normal
distribution with equal covariance matrices.    Also see
...
{phang}
{bf:LOO}.
LOO is an acronym for leave one out.  In discriminant analysis,
classification of an observation while leaving it out of the estimation sample
is done to check the robustness of the analysis.  Also see
{help mv_glossary##discrim:discriminant analysis}.
...
{phang}
{bf:MANCOVA}.
MANCOVA is multivariate
analysis of covariance.  See MANOVA.

{phang}
{bf:MANOVA}.
MANOVA is multivariate analysis of variance;
it is used to test hypotheses about means.  Four multivariate statistics are
commonly computed in MANOVA: Wilk's lambda, Pillai's trace,
Lawley-Hotelling trace, and Roy's largest root.  Also see
{help mv_glossary##Wilk:Wilk's lambda},
...
guest's feet, and if the guest was too short, he would stretching the guest out
on a rack.
...
Standardized data has a mean of zero and a standard deviation of one.   You can
standardize data {bf:x} by taking ({bf:x} - {bf:x}[bar])/sigma,
...
{marker Wilk}{...}
{bf:Wilk's lambda}.
Wilk's lambda is a test statistic for the hypothesis test H_0: {bf:mu}_1 =

...
program notes
	...
	if (`"`one'"' == substr("list", 1, max(1,strlen(`"`one'"')))) {
		notelist `rest'
		exit
	}
	if (`"`one'"' == "drop") { 
...
void splitstring(`SS' lname, `SS' rname, `SS' strname)
{
	...
	if (p   = strpos(str, " in ")) { 
		st_local(lname, strtrim(substr(str, 1, p-1)))
		st_local(rname, strtrim(substr(str, p+4, .)))
	}
	else {

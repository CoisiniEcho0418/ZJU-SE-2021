...
{p2colset 5 22 24 2}{...}
{p2col:{manlink ST Glossary} {hline 2}}Glossary of terms{p_end}
{p2colreset}{...}
...
{bf:accrual period} or {bf:recruitment period} or {bf:accrual}.
     The accrual period (or recruitment period) is the period during
     which subjects are being enrolled (recruited) into a study.  Also see
     {it:{help st_glossary##followup_period:follow-up period}}.

{phang}
{marker administrative_censoring}{...}
{bf:administrative censoring}.
     Administrative censoring is the right-censoring that occurs when the
     study observation period ends.  All subjects complete the course of the
     study and are known to have experienced either of two outcomes at the
     end of the study: survival or failure.  This type of censoring should not
     be confused with 
     {it:{help st_glossary##withdrawal:withdrawal}} and 
     {it:{help st_glossary##loss_to_followup:loss to follow-up}}.  Also see
     {it:{help st_glossary##censoring:censored, censoring, left-censoring, and right-censoring}}.
...
{phang}
{bf:case-control studies}.
    In case-control studies, cases meeting a fixed criterion are
    matched to noncases ex post to study differences in
    possible covariates.  Relative sample sizes are usually fixed at 1:1 or
    1:2 but sometimes vary once the survey is complete.  In any case, sample
    sizes do not reflect the distribution in the underlying population.
...
{phang}
{marker censoring}{...}
{bf:censored}, {bf:censoring}, {bf:left-censoring}, and
{bf:right-censoring}.
     An observation is left-censored when the exact time of the onset of risk
     is not known; it is merely known that the onset of risk occurred before
     {it:t_l}.

{pmore}
    An observation is right-censored when the time of failure is not known; it
    is merely known that the failure occurred after {it:t_r}.

{pmore}
    In common usage, censored without a modifier means right-censoring.

{pmore}
    Also see {it:{help st_glossary##truncation:truncation}}.

{phang}
{bf:CIF}.  See {it:{help st_glossary##CIF:cumulative incidence function}}.

{phang}
{marker cohort_studies}{...}
{bf:cohort studies}.
    In cohort studies, a group that is well defined is monitored over 
    time to track the transition of noncases to cases.  Cohort studies
    differ from incidence studies in that they can be retrospective as well as
    prospective.
...
{phang}
{bf:confounding}.
    In the analysis of epidemiological tables, factor or interaction effects
...
{phang}
{bf:cross-sectional or prevalence studies}.
    Cross-sectional studies sample distributions of healthy and diseased 
    subjects in the population at one point in time.

{phang}
{bf:crude estimate}.  A crude estimate is an estimate that is chosen not for
accuracy or precision but instead for ease of calculation.
...
{phang}
{bf:exponential test}.  
The exponential test is
    the parametric test comparing the hazard rates, lambda_1 and
    lambda_2, (or log hazards) from two independent exponential
    (constant only) regression models with the null hypothesis
    H_0: lambda_2-lambda_1=0 (or
    H_0: ln(lambda_2)-ln(lambda_1)=ln(lambda_2/lambda_1)=0).
...
{bf:follow-up period} or {bf:follow-up}.
    The (minimum) follow-up period is the period after the last
    subject entered the study until the end of the study.  The follow-up
    defines the phase of a study during which subjects are under
    observation and no new subjects enter the study.  If T is the total
    duration of a study, and R is the accrual period of the study, then
    follow-up period f is equal to T-R.  Also see
    {it:{help st_glossary##accrual_period:accrual period}}.
...
{phang}
{bf:incidence studies}, 
{bf:longitudinal studies}, 
and
{bf:follow-up studies}.
    Whichever word is used, these studies monitor a population for a 
    time to track the transition of noncases into cases.  Incidence studies
    are prospective.  Also see
    {it:{help st_glossary##cohort_studies:cohort studies}}.
...
{phang}
{bf:left-censoring}.
   See {it:{help st_glossary##censoring:censored, censoring, left-censoring, and right-censoring}}.

{phang}
{bf:left-truncation}.
   See {it:{help st_glossary##truncation:truncation}}.
...
{bf:loss to follow-up}.
    ...
    end of the study, had they completed the study, is unavailable.  Also see
    {it:{help st_glossary##withdrawal:withdrawal}},
    {it:{help st_glossary##administrative_censoring:administrative censoring}},
    and {it:{help st_glossary##followup_period:follow-up period or follow-up}}.

{phang}
{bf:matched case-control study}.
     Also known as a retrospective study, a matched case-control study is a
     study in which persons with positive outcomes are each matched with one
     or more persons with negative outcomes but with similar characteristics.
...
{phang}
{bf:prevented fraction}. A prevented fraction is the reduction in the risk of
a disease or other condition of interest caused by including a protective risk
factor or public-health intervention.
...
Also known as a prospective longitudinal study, a prospective study is a study
...
{phang}
{bf:risk ratio}.
In a log-linear model, this is the ratio of probability of survival associated
with a one-unit increase in a risk factor relative to that calculated without
such an increase, that is, R(x+1)/R(x).  Given the exponential form of the
model, R(x+1)/R(x) is constant and is given by the exponentiated coefficient.
...
{phang}
{bf:SMR}.
    SMR stands for standardized mortality (morbidity) ratio and is the
...
{marker truncation}{...}
{bf:truncation}.
     In statistics, truncation refers to cases where subjects are 
     unobserved (do not appear in the data).
     Censoring, on the other hand, refers to cases when subjects are 
     known to fail within a certain time span, except that the exact failure
     time is unknown.
     
{pmore}
     In survival analysis, truncation -- sometimes called
     left-truncation -- occurs when subjects are not observed before
     time {it:t}_{it:c}, only to appear in the study at time {it:t}_{it:c}.
     It is only because they didn't fail before {it:t}_{it:c} that we even
     ...
     {it:t}_{it:c}, but we just do not know exactly when.

{pmore}
     Given these conventions, right-truncation is equivalent to 
     right-censoring, because failure (at some point before time infinity) 
     is inevitable.
...
{phang}
{bf:type I study}.
     A type I study is a study in which all subjects fail (or experience an
     event) by the end of the study; that is, no censoring of subjects occurs.
...
{phang}
{bf:type II study}.
     A type II study is a study in which there are subjects who do not fail
     (or do not experience an event) by the end of the study.  These subjects
     are known to be censored.
...
{bf:withdrawal}.
    ...
    reasons unrelated to the event of interest.  For example, withdrawal
    occurs if subjects move to a different area or decide to no longer
    participate in a study.  Withdrawal should not be confused with
    administrative censoring.  If subjects withdraw from the study, the
    information about the outcome those subjects would have experienced at the
    end of the study, had they completed the study, is unavailable.  Also see
    {it:{help st_glossary##loss_to_followup:loss to follow-up}} and
    {it:{help st_glossary##administrative_censoring:administrative censoring}}.
{p_end}
